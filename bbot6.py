...
#############################################
  ...
  #  deadzone_maxangle=0
    tipped_over_angle=22  # plus or minus 20 degrees.  Kill the motor and let the robot stop
    target_angle=-3.5  # (-4) the angle the robot needs to be to balance.  The error angle is calculated from this
...
# is the robot tipped over?
          ...
          #      robotlog.write("TIPPED OVER. time="+str(timelog)+" actual angle="+str(actual_angle)+" relative angle "+str(relative_angle)+", rate:"+str(angle_rate)+" p="+str(p)+" i="+str(i)+" d="+str(d)+" PID motor speed="+str(motor_speed)+" Kp="+str(Inner_PID.Kp)+" Ki="+str(Inner_PID.Ki)+" Kd="+str(Inner_PID.Kd)+"\n")
                ...
                p,i,d,i_error=Inner_PID.PID_processor(actual_angle,target_angle,angle_rate,IKp,IKi,IKd,10,10)   #  current angle, command angle and gyro rate (as the differential element), integral number of samples, and sample rate
                motor_speed=p+i+d
                ...
                if n%20==0:   # check every 20 cycles
                    ...
                    p,i,d,s_error=Outer_PID.PID_processor(motor_speed,command_speed,0,OKp,OKi,OKd,15,1)
                #    print("outer p=",p," i=",i," d=",d)
                    target_angle=(p+i+d)+start_target_angle
            ...
            robotlog.write(" n="+str(n)+" actual a="+str(actual_angle)+" ta="+str(target_angle)+" tat "+str(target_angle_temp)+" rate:"+str(angle_rate)+" cs="+str(command_speed)+" PID  actual s="+str(motor_speed)+" ehist="+str(i_error)+" shist="+str(s_error)+"t="+str(timelog)+"\n")
