...
#################################################################
    ...
    for window_length in range(0,len(mats)):
        ...
        for col in colnames:
            ...
            table_u=add_mat(table_u,col_no,col,mats[window_length],series_dict,mat_type)
            col_no+=1
...
# def build_fast_mini_batch_input(series,no_of_batches,no_of_steps):
#     print("build fast mini batch series shape",series.shape)
#     np.random.seed(42) 
#     series_steps_size=series.shape[1]
...
#         if i%100==0:
#             print("\rFast Batch:",i,"new_mini_batch.shape:",new_mini_batch.shape,flush=True,end="\r")
...
# def add_a_new_series2(table,arr_names,arr):
#     table=table.T
...
#     finish_arr = np.empty((table.shape[0]-arr.shape[1],arr.shape[2]))
#   #  print("startarr1 shape=",start_arr.shape)
#     finish_arr[:] = np.NaN

#     new_arr=np.vstack((new_arr,finish_arr))  
...
def add_a_new_series(table,arr_names,arr,start_series_len,predict_ahead_steps):
    first_date=table.T.index[0].strftime('%Y-%m-%d')
    pidx = pd.period_range(first_date, periods=PERIODS)   # 2000 days
   # new_table = pd.DataFrame(np.nan, index=arr_names,columns=pidx)   #,dtype='category')  #series_table.columns)
   # new_table=new_table.T
   # print("add a new serioes, new_table=\n",new_table,new_table.shape)   
    print("add a new serioes, table=\n",table,table.shape)   
    ...
    print("start_series_len=",start_series_len)
   
    start_arr = np.empty((start_series_len,arr.shape[2]))
    start_arr[:] = np.NaN
    print("startarr shape=",start_arr,start_arr.shape)
  #  print("start arr shape2=",start_arr.shape)
    new_arr=np.vstack((start_arr,arr[0]))  
    print("new_arr shape=",new_arr,new_arr.shape)
 
    print("new_arr\n",new_arr,new_arr.shape)  #,table.shape[0]-arr.shape[1]-predict_ahead_steps)   

    finish_arr = np.empty((PERIODS-start_series_len-arr.shape[1],arr.shape[2]))
    print("finish_arr shape=",finish_arr.shape)
    finish_arr[:] = np.NaN

    new_arr2=np.vstack((new_arr,finish_arr))  
    print("final new_arr2\n",new_arr2,new_arr2.shape)  #,table.shape[0]-arr.shape[1]-predict_ahead_steps)   

    
 
    new_cols=pd.DataFrame(new_arr2,columns=arr_names,index=pidx)
    print("nc=\n",new_cols,new_cols.shape)
    #new_cols['pe=pidx
  #  new_cols.set_index(pidx, inplace=True)
 #   new_cols=new_cols.T
 #   new_cols['series_type']=arr_type
  #  new_cols=new_cols.T
  #  print("new_cols=",new_cols,new_cols.shape)
    table=table.T 
    print("before concat to current table table=\n",table,table.shape)
    table2=pd.concat((table,new_cols),join='outer',axis=1)  
  #  table2=table.join(new_cols,how='outer',rsuffix="_r?",sort=True) 



 #   table2.drop(table2.iloc[range(0,PERIODS-1),:],axis=0,inplace=True)
...
#    print("table2=\n",table2,table2.shape)
 ...
 #   table2 = table2.reindex(natsorted(table2.columns), axis=1) 
  ...
  #  new_product_names = table2.index.map('_'.join).astype(str)
    table2=table2.T
    print("3table2=\n",table2,table2.shape)
  #  table2.drop(table2.columns[range(PERIODS,PERIODS*2)],axis=1,inplace=True)
#    print("table2 cols=",table2.columns)
 # series_table.index = series_table.index.map('_'.join).astype(str)  
   # print("new series added. extended_series_table=\n",table2,table2.shape) 
    print("new product names=\n",new_product_names)
...
def main():
  ...
  #  PERIODS=predict_ahead_steps
 #   epochs_cnn=1
    epochs_wavenet=15
    no_of_batches=30000   #1       # rotate the weeks forward in the batch by one week each time to maintain the integrity of the series, just change its starting point
...
#    y_length=1
   ...
   # strides=2   # for CNN
    ...
    mats=[365]   #omving average window periods for each data column to add to series table
...
#    print("est before=",extended_series_table,extended_series_table.shape)  
    extended_series_table,product_names=add_a_new_series(extended_series_table,pred_product_names,ys,start_point,predict_ahead_steps)
 ...
 #   print("mat_sales_x shape",mat_sales_x.shape,"n_steps=",n_steps)
  ...
  #  mc_yerr=[0]
   ...
   # print("mc_yerr=\n",mc_yerr,mc_yerr.shape)
    ...
    extended_series_table,product_names=add_a_new_series(extended_series_table,pred_product_names,mc_ys,start_point,predict_ahead_steps)

    pred_product_names=[s + "yerr_mc" for s in original_product_names]  
    extended_series_table,product_names=add_a_new_series(extended_series_table,pred_product_names,mc_yerr,start_point,predict_ahead_steps)


    for p in range(0,extended_series_table.shape[0]):
        plt.figure(figsize=(11,4))
        plt.subplot(121)
        plt.title("Series:Actual+Pred: "+str(product_names[p]),fontsize=14)
