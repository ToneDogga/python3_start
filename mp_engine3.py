...
def prods_and_custs(sales_df):
  ...
  #   cust_list=[c for c in cust_list[2] if c!="OFFINV"]
    ...
    # cust_list=sorted(cust_list)
     return [[c,p] for c in cust_list for p in prod_list[:10]]
...
def main():   #classifier_mp_GA(params):   # multiprocessing stuff will go here later
    global sales_df  # need to do this as can only pass one parameter to multiprocesser .map
 ...
 #   print(cust_prod_list)
  ...
  #  q = manager.Queue()    
    ...
    print("Multiprocessing",len(cust_prod_list),"possible combinations of customer and product with",cpus,"cpus.\n")
