...
def next_batch(sales,product_row_no,n_steps,n_inputs):
...
def load_data(filename):   #,mask_text):   #,batch_size,n_steps,n_inputs):   
    print("Loading excel file",filename)
    ...
    print("Creating Sales Pivot Table...")
  #  mask=((df["product"]=="SJ300") | (df["product"]=="BBQ300"))
    mask=((df["product"]=="SJ300") & (df["code"]=="FLPAS"))

  #  mask=(df["product"]=="SJ300")
    table = pd.pivot_table(df[mask], values='qty', index=['code','product'],columns=['week'], aggfunc=np.sum, margins=False, fill_value=0)   #, observed=True)
...
def main():  
  ...
  #  n_outputs=1
    learning_rate=0.001
    n_iterations=1200
 #   product_row_no=np.array([0,1],dtype=int)   product_row_no=0
    n_predict_steps=80
    t_start=3
    t_finish=102
 ...
 #   n_start_inputs=sales.shape[1]
   # total_size=n_inputs*n_steps
    ...
    for product_row_no in range(0,n_rows):
        ...
        plt.subplot(121)
        plt.title("A Unit sales series: "+str(product_names[product_row_no]),fontsize=14)
...
###############################################3
        ...
        product_no_vector=np.array(np.arange(0,n_rows,dtype=int)) 
        
        print("product no vector=",product_no_vector)
    ...
    #    outputs, states = tf.nn.dynamic_rnn(cell, X, dtype=tf.float32)
     ...
     #   product_row_no=np.array([0,1],dtype=int)
        ...
        with tf.Session() as sess:
            ...
            for iteration in range(n_iterations):
                X_batch, y_batch = next_batch(sales,product_row_no,n_steps,n_inputs)
          #      print("X_batch=\n",X_batch,"\ny_batch=\n",y_batch)
          #      print("X_batch=\n",X_batch.shape,"\ny_batch=\n",y_batch.shape)
...
################################################3                    
 ...
 #   for product_row_no in range(0,n_rows):
        ...
        plt.title("Testing the model: "+str(product_names[product_row_no]),fontsize=14)
    ...
    #    plt.plot(x_axis[1:], sales[0,x_axis[1:]], "w*", markersize=10, label="target")
       ...
       # print("3 y_pred=",y_pred)   #[:,-n_steps:,:]
        ...
        plt.plot(x_axis[3:]+1, y_pred[0,2:,0], "r.", markersize=10, label="prediction")
 ...
 #   for product_row_no in range(0,n_rows):
        ...
        #    print("pnv shape=",product_no_vector.shape)
      #     sales[product_row_no,:-1].reshape(-1,n_steps,n_inputs) 
            sequence=sales[product_row_no,:-1]   #.reshape(1,product_no_vector.shape[0],-1)
      #      print("start sequence=",sequence,sequence.shape)
       #     print("prodcut row no=",product_row_no) 
           #nexts=sequence[-1]+1
           #print("sq=",sequence,"nexts",nexts,"n_steps=",n_steps)
       
            for iteration in range(0,n_predict_steps):
               X_batch=sequence[-n_steps:].reshape(-1, n_steps, n_inputs)
              # X_batch = np.array(sequence[0,-training_length:]).reshape(1,training_length,n_inputs)
               y_pred = sess.run(outputs, feed_dict={X: X_batch})
            #   print("x_batch=\n",X_batch.shape,"y_pred=\n",y_pred.shape)
         #      print("i=",iteration,"y_pred=",y_pred[0,-1,0])
               sequence=np.append(sequence,y_pred[0, -1, 0])    #,axis=1)
         #      print("Sequence:",iteration,sequence.shape)    
     ...
     #   for product_row_no in range(0,n_rows):
        ...
        plt.plot(pred_axis, sequence[-n_predict_steps:], "r-",linewidth=3,label=r"$unit sales$")
   ...
   #     print(sequence)
        with open("sales_prediction_"+str(product_names[product_row_no])+".csv", 'w') as csvfile:
             s_writer=csv.writer(csvfile) 
             s_writer.writerow(sequence)
 

        #    for line in sequence:
         #        s_writer.writerow(line)
         #    s_writer.writerows(sequence)
