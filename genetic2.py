    ...
    for elem in dna:
        ...
        if direction=="x":   # maximise
         ...
         #   print("#",count+1,":",elem," val=",val,"payoff=",payoff[val]," prob=",wheel[count])
            ...
            wheel.append(int(round((total_payoff-payoff[val])/total_payoff*1000)))
...
print(dna)
print("Running for ",epoch_length," generations.")
while generation_number<=epoch_length:
    print("\rGeneration progress: [%d%%]  " % (generation_number/epoch_length*100), end='\r', flush=True)
    #sys.stdout.write("Generation: %d%%   \r" % (generation_number/epoch_length*100) )
    #sys.stdout.flush()
   # str1="Generation: [%d%%]".format(generation_number/epoch_length*100)
   # back='\b'*len(str1)
   # print(back, end="")
   ## print(str1 %, end="")
   # perc = int(generation_number/epoch_length*100)
   # linestr="Epoch is [%d%%] complete     " % perc
   # print(linestr,end="",flush=True),   # end='\r', flush=True)
   # print('\x08',end="",flush=True)
    
   # sys.stdout.write("Generation:"+str(generation_number)+"\r")
    ...
    if direction=="x":
       ...
       if returned_payoff<min_payoff:
            ...
            print("best fittest=",best," generation no:",gen," min_cost=",min_payoff)
    else:
    ...
    dna=mutate(crossed,length,1000)   # 1000 means mutation 1 in a 1000 bits processed
    generation_number+=1
