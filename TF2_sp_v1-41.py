...
################################################    
    ...
    # norm_sales=tf.keras.utils.normalize(sales, axis=-1, order=2)
    # norm_mat_sales=tf.keras.utils.normalize(mat_sales, axis=-1, order=2)
    # norm_mat_sales_90=tf.keras.utils.normalize(mat_sales_90, axis=-1, order=2)
...
def load_shop_data(filename):   #,mask_text):   #,batch_size,n_steps,n_inputs):   
 ...
 #   df['productgroup']=df['productgroup'].astype('category')
  ...
  #  table = pd.pivot_table(df, values=['salesval'],index=['product'],columns=['period'], aggfunc=[np.sum], margins=False,dropna=False, fill_value=0)   #observed=True
    table = pd.pivot_table(df[mask], values=['qty'],index=['product'],columns=['period'], aggfunc=[np.sum], margins=False,dropna=False, fill_value=0)   #observed=True
...
def mat_convert(table,window_period):
   #   print("table iloc[:window period]",table.iloc[:,:window_period])     #shape[1])
      start_mean=table.iloc[:,:window_period].mean(axis=1)   # axis =1
    #  print("table iloc=",table.iloc[:window_period].shape,"\n startmean=\n",start_mean)
      rtable= table.T.rolling(window=window_period,axis=0).mean()
      rtable=rtable.fillna(start_mean)
    #  print("rtable shape=",rtable.shape)   #,rtable.iloc[:window_period,0],"rtable.columns",rtable.columns,"rtable shape=",rtable.shape,"start mean=",start_mean)
      return rtable
...
class MyCustomCallback(tf.keras.callbacks.Callback):
  ...
  def on_train_begin(self, logs=None):
    print('Training:  begins at {}'.format(datetime.datetime.now().time()))

  def on_train_end(self, logs=None):
    print('Training:  ends at {}'.format(datetime.datetime.now().time()))

  def on_predict_begin(self, logs=None):
    print('Predicting: begins at {}'.format(datetime.datetime.now().time()))

  def on_predict_end(self, logs=None):
    print('Predicting: ends at {}'.format(datetime.datetime.now().time()))
...
def main():
   ...
   # n_steps = 100
    ...
    batch_length=4  # one week=5 days   #4   #731   #731  #365  3 years of days  1096
    y_length=1
    neurons=220
    start_point=20
    plot_y_extra=1   # extra y to graph plot 
 #   mat_length=20
...
#    mask="(df['product']=='SJ300')"
  ...
  #  print("mat_sales_90 shape",mat_sales_90.shape)
    ...
    x_axis=np.arange(-n_steps,predict_ahead_steps+1,1)
    ...
    for p in range(0,series2.shape[0]):
       ...
       #plt.title("A unit sales series", fontsize=14)
        ...
        plt.plot(range(0,series2.shape[1]), series2[p,:], "b-",alpha=1,label=r"$Units$")
    ...
    #   plt.plot(x_axis[:-1], sales[product_row_no,x_axis[:-1]], "b-", linewidth=3, label="A training instance")
        ...
        plt.axis([0, series2.shape[1]+predict_ahead_steps+1, 0, np.max(series2[p])+plot_y_extra])
        plt.xlabel("Period")
        plt.ylabel("dollars")
...
###########################################################################################
        ...
        for rate in (1,2,4,8) *2:
            ...
            if (layer_count==7):    # | (layer_count==7):
    ...
    #    callbacks = [tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=10),MyCustomCallback()]
        callbacks = [tf.keras.callbacks.EarlyStopping(monitor='loss', patience=10),MyCustomCallback()]
...
#################################################################    
 ...
 #   print("mat sales shape",mat_sales.shape)
    ...
    for p in range(0,n_query_rows):
        ...
        plt.plot(range(sales.shape[1]-1,sales.shape[1]+predict_ahead_steps-1), ys[0,-(predict_ahead_steps):,p], "r-", markersize=5, label="prediction")
 ...
 ############################################################################3
    ...
    print("mat sales 90 shape=",mat_sales_90.shape)
    n_steps=mat_sales_90.shape[1]
  ...
  #   X_new,Y_new=build_mini_batch_input(series2,no_of_batches,n_steps+1)
    ...
    print("mat sales 90",mat_sales_90)
...
#          y_probas=np.stack([model(X_test,training=True) for sample in range(3)])
          y_probas=np.stack([model.predict(X_test,training=True) for sample in range(3)])
          ...
          mat_sales_90=np.append(mat_sales_90[0,:],y_mean[:,-1]).reshape(1,-1,n_query_rows)
          print("saehad loop mat sales 90=",mat_sales_90.shape,"n steps=",n_steps)
          X_test,y=build_mini_batch_input(mat_sales_90[-n_steps:],10000,batch_length)
 ...
 #          print("Building step:",step_ahead,"/",pas)  
 #      #    X_new=X[:,(-n_steps+step_ahead):,:]
 #          y_probas=np.stack([model(X_test,training=True) for sample in range(20)])
 #          y_mean=np.round(y_probas.mean(axis=0),2)
 #          y_stddev=np.round(y_probas.std(axis=0),2)
 #      #    print("\rstep:",step_ahead,"/",pas,"y mean=",y_mean,"y mean shape=",y_mean.shape,"y stddev=",y_stddev,"y stddev shape=",y_stddev.shape,end='\r',flush=True)
 #          print("\rstep:",step_ahead,"/",pas,"y mean shape=",y_mean.shape,"y stddev shape=",y_stddev.shape,end='\r',flush=True)
...
#################################################################    
 ...
 #   print("mat sales shape",mat_sales.shape)
    ...
    # print('\n# Evaluate y_pred on test data')
    # results = model.evaluate(X_test, y_mean, batch_size=4)
    # print('test loss, test acc:', results)

       
    # history = model.fit(X_test, y_mean, epochs=epochs_wavenet)
    # print("loss history=",history.history["loss"])
    #    #, callbacks=callbacks,
    #     #                    validation_data=(X_valid, y_valid))
