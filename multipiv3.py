...
class multipi:
            ...
            for event in pygame.event.get():
                  if event.type == pygame.KEYDOWN: 
                        ...
                        if k=="left ctrl":
                            ...
                            mymsg2=self.append_alias(mymsg,alias_name)  # 10 chars
                            mymsg3=self.append_hash(mymsg2)
                            #print("mymsg after hash=",mymsg)
                            #enc=e.encrypt(mymsg3)
                            #print('\n'+"server broadcasting msg=",mymsg," encrypted=",enc)
                        
                            self.broadcast(socket_list,server_socket,"",e.encrypt(mymsg3))
                            mymsg=""
                            mymsg2=""
                            mymsg3=""
                            me_print=False
            ...
            for sock in ready_to_read:
                ...
                if sock == server_socket: 
                   ...
                   # print("Client (%s, %s) connected" % addr)
                    ...
                    print(msg)
                    msg2=self.append_alias(msg,alias_name)  # 10 chars
                    msg3=self.append_hash(msg2)   #.encode('utf-8'))
                    #msg2=e.encrypt(msg)
                    #print("msg=",msg)
                    me_print=False
                 
                    self.broadcast(socket_list,server_socket, sockfd, e.encrypt(msg3))
                        ...
                        if data:
                           ...
                           # print("there is data")
                            ...
                            try:
                                ...
                                sys.exit()
                            dec2,success,client_alias,hash_bytes=self.unpack_hash(dec)
                        #    print("client alias=",client_alias)
                      #      print("decrypted=",dec)

                            #client_alias=client_alias.rstrip()    
                            if success:
                           #     print("hash correct")
                                #print(str(sock.getpeername()),"data = ",dec)
                                print("{"+client_alias+"}:",dec2)
                                msg=dec2
                                me_print=False

                            else:
                                me_print=False
                                msg="hash incorrect hash=" + str(hash_bytes)+" : "+str(sock.getpeername()) + ":" + dec2
                                print(msg) #.rstrip())
                            msg2=self.append_alias(msg,client_alias)  # 10 chars
                            msg3=self.append_hash(msg2)
                            self.broadcast(socket_list,server_socket, sock, e.encrypt(msg3))
                        else:
    ...
    def chat_client_encrypted(self,e,alias_name):    #,hasher):   # e is the AES cipher
        ...
        if(len(sys.argv) < 4) :
            sys.stdout.write("Usage : python3 encrypted_chat_client1.py hostname port passphrase\n")
            sys.exit()
        ...
        sys.stdout.write("["+alias_name+"] Connected to remote host. You can start sending messages.\n")
...
#                            print(str(sock.getpeername())," : ",data," hash=",hash_msg(data))
                           ...
                           # print("decrypt=",dec2," hash bytes",hash_bytes2)
                            ...
                            if not success:
                                sys.stdout.write("\nHash incorrect: hash="+str(hash_bytes2)+" : "+str(sock.getpeername())+" : "+dec2+"\n")
                                sys.stdout.flush()
                                  #  sys.stdout.write(str(sock.getpeername())+" : "+data)
                            else:
                                sys.stdout.write("{"+client_alias+"} : "+dec2)   #+str(sock.getpeername())
                                sys.stdout.flush() 
                       ...
                       # print("exception")
                        m="["+alias_name+"] Error exception: Client (%s, %s) is offline" % addr
                        sys.stdout.write(m) ; sys.stdout.flush()
                        msg=self.append_alias(m,alias_name)  # 10 chars
  ...
  #     hash_frame=(hashlib.md5(msg.encode('utf-8'))).hexdigest()  #.digest
    ...
    def unpack_hash(self,byte_frame):
        ...
        if length>74:
            ...
            a=str(byte_frame[-74:])
            alias_name=a[:10]
            hash_msg=str(byte_frame[:length-64])   #,'utf-8')
            msg=str(byte_frame[:length-74])   #,'utf-8')

  #         check_hash=(hashlib.md5(msg.encode('utf-8'))).hexdigest()  #.digest

            check_hash=(hashlib.sha256(hash_msg.encode('utf-8'))).hexdigest()  #.digest
    
            if hash_bytes!=check_hash:
                sys.stdout.write("hash incorrect: actual hash= "+hash_bytes+" check="+check_hash)
                sys.stdout.flush() 
                return(msg,False,alias_name,hash_bytes)  #"hash incorrect")

            else:
                #print("hash correct  Frame_no:",frame_count)
                return(msg,True,alias_name,hash_bytes)
        else:
            sys,stdout.write("byte frame error.  len="+length+"<=74.  byte_frame="+byte_frame)
            sys.stdout.flush() 
            return("",False,"",b'')
