...
#    table.drop(columns=['glset'],axis=0)
 ...
 #   sales=sales[...,:n_steps]
 #   print("sales=\n",sales,sales.shape)        
    return sales[..., np.newaxis].astype(np.float32),product_names,dates,table
...
def main():
   ...
   # n_steps = 100
    predict_ahead_steps=50
    epochs_cnn=1
    epochs_wavenet=100
    no_of_batches=200000   #1       # rotate the weeks forward in the batch by one week each time to maintain the integrity of the series, just change its starting point
    batch_length=5   #731   #731  #365  3 years of days  1096
    y_length=1
    neurons=101
...
#    mask="(df['product']=='SJ300')"
    ...
    #    series2,product_names,ddates,table=load_shop_data(filename)    #,n_steps+1)  #,batch_size,n_steps,n_inputs)
        series2,product_names,ddates,table=load_data(filename)    #,n_steps+1)  #,batch_size,n_steps,n_inputs)
