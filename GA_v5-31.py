        ...
        for gene in range(0,r.pop_size):
          ...
          #  val=int(fittest,2)   # binary base turned into integer
            if r.row_find_method=="c":
                ...
                total_payoff+=abs(float(return_a_row_from_file(int(fittest,2),r.payoff_filename,r)[8].rstrip()))
            else:
                pass
           # count+=1
...
#            val=int(fittest,2)   # binary base turned into integer
            if r.row_find_method=="c":
                ...
                p=abs(float(return_a_row_from_file(int(fittest,2),r.payoff_filename,r)[8].rstrip()))
            else:
                pass
         ...
         #   print("Epochs:",epoch+1,"Generation progress: [%d%%]" % (generation/epoch_length*100),"diversity (len_sel)=",len_sel,"Tot gene bits:%d" % total_genes,"Tot Mutations:%d      "  % (mutations), end='\r', flush=True)
          ...
          #      val=int(fittest,2)   # binary base turned into integer
                ...
                try:
                    ...
                    if r.row_find_method=="c":
                        plist=return_a_row_from_envir_using_concatno(fittest,env)
                    elif r.row_find_method=="r":
                        plist=return_a_row_from_envir_using_rowno(int(fittest,2),env)
                    elif r.row_find_method=="s":
                        plist=return_a_row_from_file(int(fittest,2),r.payoff_filename,r)
...
# length in bits of each input variable
    ...
    ga.alen=3   # length in binary bits of each integer input
    ga.blen=3
    ga.clen=3
    ga.dlen=3
    ga.elen=3
...
#   load filename.csv into pandas for fast analysis
    ...
    print("\n")
    ga.row_find_method=""
    while ga.row_find_method!="c" and ga.row_find_method!="r" and ga.row_find_method!="s":
        ga.row_find_method=input("Use (c)oncatno or (r)owno or file(s)eek?")
