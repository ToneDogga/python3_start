...
def calibrate_gyro(rp):
    starting_angle=0
    print("Set robot up ready to balance. wait...")
...
#############################################
   ...
   # target_angle=0  # (-4) the angle the robot needs to be to balance.  The error angle is calculated from this
    ...
    motor_bias=1.05    # motor C moves faster than motor B.  add an extra bit of speed
    ...
    # Speed loop PID constants  speed error, adjust the target angle every 20 cycles
    SKp=0.042      # (0.042) (0.012)proportional constant 
    SKi=0.015     # (0.01)integral constant(x10) 0.0005 
    SKd=0      # (0.0)differential constant 


    # Position loop PID constants  position error, adjust the target angle every 60 cycles
    PKp=0.005     # (0.005) proportional constant 
    ...
    position_adjustment_angle=0
    speed_adjustment_angle=0
...
##################################################
 ...
 #############################333
   ...
   # target_angle=start_target_angle+balance_bias_angle
...
# this goes through another different PID which processes the speed error and adjusts the command angle
          ...
          # get gyro angle
            try:
                angle=gyro_angle(BP)   # returns a tuple, angle and rate of change
            except IOError as error:
                print(error)
         ...
         #   time.sleep(0.02)
            ...
            target_angle=start_target_angle+speed_adjustment_angle+position_adjustment_angle
...
# is the robot tipped over?
          ...
          #      robotlog.write("TIPPED OVER. time="+str(timelog)+" actual angle="+str(actual_angle)+" relative angle "+str(relative_angle)+", rate:"+str(angle_rate)+" p="+str(p)+" i="+str(i)+" d="+str(d)+" PID motor speed="+str(motor_speed)+" Kp="+str(Inner_PID.Kp)+" Ki="+str(Inner_PID.Ki)+" Kd="+str(Inner_PID.Kd)+"\n")
            ...
            if n%7==0:   # check every 10 cycles
 

                    
                speed_history.append(motor_speed)   #  keep the every 10th error in order to sum them for the integral (i)
                speed_list_length+=1
                if speed_list_length>20:   # list of 6
                    speed_list_length-=1
                    del speed_history[0]
                ave_motor_speed=sum(speed_history)/20

                p2,i2,d2,s_error=Speed_PID.PID_processor(ave_motor_speed,command_speed,0,SKp,SKi,SKd,10,7)  # 10,7
                #    print("outer p=",p," i=",i," d=",d)
               # target_angle=-((p2+i2+d2)-start_target_angle)+balance_bias_angle
                speed_adjustment_angle=-(p2+i2+d2)
              #  target_angle-=speed_adjustment_angle

                if speed_adjustment_angle>5:
                    speed_adjustment_angle=5
                elif speed_adjustment_angle<-5:
                    speed_adjustment_angle=-5
            ...
            if n%60==0:   # check every 100 cycles
              #  if random.randint(1,2)==1:
                position = (BP.get_motor_encoder(BP.PORT_B)-BP.get_motor_encoder(BP.PORT_C))/2
               # else:
                #    position = (-BP.get_motor_encoder(BP.PORT_C)+BP.get_motor_encoder(BP.PORT_B))/2
                position_history.append(position)   #  keep the every 10th error in order to sum them for the integral (i)
                position_list_length+=1
                if position_list_length>6:   # list of 20
                    position_list_length-=1
                    del position_history[0]
                ave_position=sum(position_history)/6

                if ave_position>500:
                    ave_position=500
                elif ave_position<-500:
                    ave_position=-500

                p3,i3,d3,p_error=Position_PID.PID_processor(ave_position,command_position,0,PKp,PKi,PKd,10,6)  # Was 10,7

                position_adjustment_angle=(p3+i3+d3)
               # if position_adjustment_angle>1:
               #     position_adjustment_angle=1
               # elif position_adjustment_angle<-1:
               #     position_adjustment_angle=-1
    ...
    #               print("set motor speeds here;  speed=",motor_speed)
                ...
                if random.randint(1,2)==1:
                    ...
                    BP.set_motor_power(BP.PORT_B ,- motor_speed+turn)   # set_motor_speed.  correct for motor bias
                else:
      ...
      #      print("motor speed=",motor_speed,"target angle",target_angle," ave speed=",ave_motor_speed," speed histyr=",speed_history)
          ...
          #  robotlog.write(" n="+str(n)+" actual a="+str(actual_angle)+" PID  actual s="+str(motor_speed)+" ta="+str(target_angle)+" tat "+str(target_angle_temp)+" rate:"+str(angle_rate)+" cs="+str(command_speed)+" p="+str(p)+" i="+str(i)+" d="+str(d)+" ehist="+str(i_error)+" shist="+str(s_error)+"t="+str(timelog)+"\n")
            robotlog.write(str(actual_angle)+","+str(speed_adjustment_angle)+","+str(position_adjustment_angle)+","+str(target_angle)+","+str(p)+","+str(i)+","+str(d)+"\n")
