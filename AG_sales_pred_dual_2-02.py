...
# sequence_lwngth_batch= np.array[2,1,2,1]
  ...
  #  print("batch series=\n",batch_series,batch_series.shape)
    indexing_series=np.empty(dtype=int,shape=(n_inputs,))
    for i in range(0,n_inputs):
       batch_series[0,:,i] = np.random.randint(0,n_steps-1,size=(n_steps))
       ...
       indexing_series=batch_series[:,:,i].reshape(-1,batch_size*n_steps)
       print("indexing series=\n",i,sales[i,indexing_series])  

    print("sales=",sales.shape)
    #ys = sales[0,indexing_series]     
    #print("ys=\n",ys,"ys.shape=",ys.shape)
