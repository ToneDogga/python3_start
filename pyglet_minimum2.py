...
def _setup_buttons(filename,sales_df_dict):
     ...
     for index, row in bdf.iterrows():
         ...
         button=button_object(name=str(row['name']),x_start=int(row['x_start']),x_len=int(row['x_len']),y_start=int(row['y_start']),y_len=int(row['y_len']),colour=colour,pushed_colour=pushed_colour,title=str(row['title']),active=bool(row['active']),visible=bool(row['visible']),pushed=bool(row['pushed']),toggle=bool(row['toggle']),button_array=np.array(np.zeros((4,4),dtype=float)),button_df=pd.DataFrame([]))    
         BUTTON_LIST.append(button)
         
     x_start=0   
     y_start=990
     for fields in sales_df_dict.keys():
         ...
         button=button_object(name=str(fields),x_start=x_start,x_len=10,y_start=y_start,y_len=30,colour=colour,pushed_colour=pushed_colour,title=str(fields),active=True,visible=True,pushed=False,toggle=True,button_array=np.array(np.zeros((4,4),dtype=float)),button_df=pd.DataFrame([]))    
         x_start+=50
         y_start-=10
         BUTTON_LIST.append(button)
    
     _resize_buttons(window.x_max,window.y_max)

     return BUTTON_LIST



def _resize_buttons(x_max,y_max):
   # batch = pyglet.graphics.Batch()
...
def main():
    ...
    BUTTON_LIST=_setup_buttons('./dash2/buttons.csv',st.sales_df_dict)
