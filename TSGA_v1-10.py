...
# there are ts.poolsize journeys
       ...
       # print("genepool dist=",ts.totalgenepool_dist)
        if totalgenepool_dist<bestjourneydist:
          ...
          #  print("best journey=",ts.genepool[bestjourneyno],"dist=",bestjourneydist)
            tsout.write("best journey= "+str(ts.genepool[bestjourneyno])+" dist="+str(round(bestjourneydist,2))+" gen:"+str(gencount)+"\n")
        print("\rGeneration:",gencount,end='\r',flush=True)
    ...
    #    print("prob table=",ts.probability_table,"sum=",sum(ts.probability_table))
        ...
        if ts.probtable_len<5:
            print("Warning, increase scaling.  Probability table too small.   <5 ")
        elif ts.probtable_len>500:
            print("WARNING, decrease scaling, Probability table too big.  > 500")
    ...
    #    print("PMX crossover")
        ...
        ts.genepool=mutate(ts.genepool,5)   # mutation rate is numbner of mutations per call
    ...
    for i in range(0,len(ts.genepool[bestjourneyno])):
        ...
        tsout.write("move ("+str(i)+") :"+str(ts.stops[ts.genepool[bestjourneyno][i]])+"\n")
        i+=1
