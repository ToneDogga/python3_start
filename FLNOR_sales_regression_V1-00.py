...
def main():
    ...
    product_encode_save="product_encode_save.p"
    LY_product_encode_save="LY_product_encode_save.p"

    scaler_save="scaler.p"
    SGDR_save="SGDRegressor.p"
    LSVR_save="LinearSVRegressor.p"
    SVR_save="SVRegressor.p"
    RFR_save="RFRegressor.p"
...
##################################################33
  ...
  #  X_df["glset_encode"] = label_encoder.fit_transform(X_df["glset"].to_numpy())
    X_df=week_delta(X_df)
    X_df=order_delta(X_df)
    X_df.dropna(inplace=True)
    X_df.drop(columns=["product","LY_product","date"],inplace=True)
...
##        print('Score with the data set of top {0} features: {1:.2f}'.format(k, score_k_features))
  ...
  #  print("X_test=\n",X_test[:10,:],"y_predict:",predictions[:10].reshape(-1,1))
    j=pd.DataFrame(np.hstack((X_test,predictions.reshape(-1,1))),columns=["LY_qty","prod_encode","LY_prod_encode","week_delta","week_of_year","order_delta","predict_qty"])
 #  print("j=\n",j)

    product_encode_save=joblib.load(open("product_encode_save.p","rb"))
    LY_product_encode_save=joblib.load(open("LY_product_encode_save.p","rb"))

    j["product"]=product_encode_save.inverse_transform(j["prod_encode"].to_numpy().astype(int))
    j= j[["product","week_of_year","order_delta","predict_qty"]]  
    j.sort_values(by=["week_of_year"],axis=0,ascending=True,inplace=True)
