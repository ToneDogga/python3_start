...
#       Visualisation with correlation matrices, histograms and scatter plots
#       
#   Feature ranking
...
def cleanout_slow_sellers(df,n):
  ...
  #  print("n=",n,"=>,",c)
    ...
    h_df=pd.DataFrame(h,columns=["prod_encode"])
    encoder=joblib.load(open(cfg.product_encode_save,"rb"))
 #   h_df["product"]=encoder.inverse_transform(h_df["prod_encode"].astype(int).to_numpy())
    h_df["product"]=encoder.inverse_transform(h_df["prod_encode"].astype(int).to_numpy())

    print("product to be removed from dataset=\n",h_df["product"])
    j=0
    for i in h:
...
##    return final_scaled,length,correction_factor
##
####
...
##       # prodrow=np.array([prod])+brkdwn
##     #   prodrow=np.insert(brkdwn, 0,prod)
##        
##      #  print(prodrow)
##     #   print("prod=",prod,"sum brkdwn",brkdwn.sum(),"brkdwn shape=",brkdwn.shape)
##        smoothing.append(brkdwn)
##       # smoothing.append(prodrow)
##
...
##
####    sarraydf = pd.DataFrame (s)
####
###### save to xlsx file
####    print("pre-Scaler array saved to",cfg.scalerdump1)
####    sarraydf.to_excel(cfg.scalerdump1, index=False)
##
##
##   # print(s[0:100])
##    # scale between zero and one.
##   # scale( s, axis=1, with_mean=False, with_std=True, copy=False )
##  #  min_max_scaler = minmax_scale(axis=1)  # axis=1 means scale each sample, axis=0 means independently scale each feature
##   # min_max_scaler = MinMaxScaler()  
##   # scaled = min_max_scaler.fit_transform(s)
##
##    scaled,active_bins,correction_factor=column_scale(s)  # for each row which is a prod)encode number, multiply by the number of active bins  and then scale between 0.5 and 1.5
##
##   
...
##    sdf.to_excel(cfg.scalerdump, index=False)
##
...
#################################
    ...
    df3["bin_no"]=pd.cut(df3["day_delta"],bins=cfg.bins,include_lowest=True,labels=range(len(cfg.bins)-1))

#    print(df3)



##    print("last_order_upspd df saved to","testing1.xlsx")
##    df3.to_excel("testing1.xlsx", index=False)
   
###################################

##    endloop=False
##    while not endloop:
##        answer=str(input("Overwrite Scaler array by product? (y/n)"))
##        endloop=(answer=="y" or answer=="n")
##    if answer=="n":    
##        scaled, active_bins,correction_factor=smoothing_distributions(df2,product_list,cfg.bins)
##    elif answer=="y":
##        print("add scaling...Load scaler array by all customers by product xlsx",cfg.scalerdump)
##        scaler_df=pd.read_excel(cfg.scalerdump,header=0,index_col=None)
##        print(scaler_df.columns)
##        scaled=scaler_df.to_numpy()
##        print(scaled)
 ...
 #   print("len scaler",len(scaler))
  ...
  #  input("?")
    ...
    for prod in product_list:
     ...
     #   binnumber_array=df3["bin_no"].to_numpy()   #.tolist()    #columns[1:].tolist()
     ...
     #   print("len=",df3[prod_mask].shape,"\ndf3=\n",df3[prod_mask])
      ...
      #      print("abin=",abin,"j=",j,"scaler=",scaler[k,abin])
  ...
  #  print(df3)
##    print("Scaler array saved to","testing2.xlsx")
##    df3.to_excel("testing2.xlsx", index=False)
...
## save to xlsx file
##    print("Scaler array saved to","testing3.xlsx")
##    df3.to_excel("testing3.xlsx", index=False)
...
####################################3
    # stack data into the future to reach 80 weeks from the newest date
    future_df=df.iloc[:,0:17]
    b4=future_df.shape[1]
    future_df.drop(columns=cfg.excludecols,inplace=True)

    print("Columns deleted:",b4-future_df.shape[1]) 

    print("Delete rows for productgroup <10 or >15.")
    b4=future_df.shape[0]
    future_df.drop(future_df[(future_df["productgroup"]<10) | (future_df["productgroup"]>15)].index,inplace=True)
    print("Rows deleted:",b4-future_df.shape[0])
    

    #print("Prepare data....")
    future_df.dropna(inplace=True)

    future_df.sort_values(by=["date"],axis=0,ascending=[False],inplace=True)
    min_date=future_df.date.min()
    max_date=future_df.date.max()
    curr_date=dt.datetime.now()
    print("min date=",min_date,"max_date=",max_date,"curr date=",curr_date)
    # create predictions 
    one_year_previously = (max_date+relativedelta(years=-1))   #.strftime('%Y/%m/%d')
    print("one year previously",one_year_previously)

    mask=((df["date"]>=one_year_previously) & (df["date"]<=max_date))
    previous_year=future_df[mask]
    new_year=previous_year.copy(deep=True)
    previous_year.rename(columns={"date": "old_date"},inplace=True)
    print("new year=\n",new_year)
    print(new_year.shape,new_year.columns)
    new_year["date"]=previous_year["old_date"]+ pd.DateOffset(years=1)
    print("new year=\n",new_year.head(50))
    print("new year=\n",new_year.tail(50))
    print(new_year.columns)
    print(new_year.shape)

   # future_df=new_year+future_df
    print("future year=\n",future_df.head(50))
    print("new year=\n",future_df.tail(50))
    print(future_df.columns)
    print(future_df.shape)
    
    bigdata = new_year.append(future_df, ignore_index=True)
    print("bigdata=\n",bigdata.head(50))
    print("bigdata=\n",bigdata.tail(50))
 ...
 #   now_dt = datetime.strptime(now, '%Y/%m/%d').date()
  #  print("now=",now)
  #  gobackweek=max_date+relativedelta(weeks=-cfg.backshift)
 #   future_df['day_delta'] = (future_df.date-future_df.date.min()).dt.days.astype(int)
   # print("future_df=\n",future_df.head(50))
   # print("future_df=\n",future_df.tail(50))

   # future_df["bin_no"]=pd.cut(future_df["day_delta"],bins=cfg.bins,include_lowest=True,labels=range(len(cfg.bins)-1)).astype(int)
   # min_bin_no=future_df.bin_no.min()
   # max_bin_no=future_df.bin_no.max()
   # print("min bin no=",min_bin_no,"max_bin_no=",max_bin_no)
   # print("future_df=\n",future_df.head(50))
   # print("future_df=\n",future_df.tail(50))

   # print("future df shape",future_df.shape)

##    start_bin_no=max_bin_no-cfg.backshift
##    if start_bin_no < min_bin_no:
##        start_bin_no=min_bin_no
##    print("start bin no=",start_bin_no)
##    print("min bin no=",min_bin_no,"max_bin_no=",max_bin_no)
##
##    mask=((future_df["bin_no"]>=start_bin_no) & (future_df["bin_no"]<=max_bin_no))
##    cols=future_df.columns
##    oldslice=future_df[mask].to_numpy()
##    current=future_df.to_numpy()
##    new_array=np.vstack((oldslice,current))
##    new_df=pd.DataFrame(new_array,columns=cols)
##    old_df=pd.DataFrame(oldslice,columns=cols)
##
##    print("oldslice_df=\n",old_df.head(50))
##    print("oldslice_df=\n",old_df.tail(50))
##    print("oldslice df shape",old_df.shape)
##
##    
##    print("new_df=\n",new_df.head(50))
##    print("new_df=\n",new_df.tail(50))
...
##    f.write("\nFrequency of products:"+str(counts)+"\n")   #dict(zip(unique, counts)))
    ...
    sr_counts=Counter(Xr_df.salesrep)   #.unique()
    print("\nFrequency of Sales Rep transactions :",sr_counts)   #dict(zip(unique, counts)))
    f.write("\nFrequency of Sales Rep transactions:"+str(sr_counts)+"\n\n")   #dict(zip(unique, counts)))
...
##    sns.distplot(pc["y"],kde=False, bins=200,rug=True)
##    plt.xlabel("Product Code")
##    plt.ylabel("Frequency")
##    plt.title("Product code frequency as a % of total transactions")
##    plt.show()
##
##    pg=pd.DataFrame(np.array(list(pg_counts.items())),columns=["x","y"])
##  #  pg.insert(0,"x",pd.Series(range(0,len(pg))))
##  #  pg.drop(columns=["c"],inplace=True)
##  #  pgs=pg[pg["y"]]
## #   print("pg=",pg)
##   # print("pgs=",pgs)
##    #pdf=pd.DataFrame(pc).hist()
##    sns.distplot(pg["y"],kde=False,bins=6,rug=True)
##  #  plt.show()
##    
##  #  sns.distplot(pg["y"],kde=False,bins=6,rug=True)
##  #  sns.distplot(pg)
...
##    plt.ylabel("Frequency")
##    plt.title("Product group frequency as a % of total transactions")
##    plt.show()
##
##
##    ccf=pd.DataFrame(np.array(list(ccode_counts.items())),columns=["x","y"])
##    #pg=pd.DataFrame(np.array(list(c_counts.items())),columns=["c","y"]).astype(int)
##  #  ccf.insert(0,"x",pd.Series(range(0,len(ccf))))
##  #  ccf.drop(columns=["c"],inplace=True)
##  #  print("ccf=",ccf)
##    sns.distplot(ccf["y"],kde=False,bins=100,rug=True)
##    plt.xlabel("Customer Code")
##    plt.ylabel("Frequency")
##    plt.title("Customer code frequency as a % of total transactions")
##    plt.show()
##
##
##    #cc=pd.DataFrame(np.array(list(ccode_counts.items())),columns=["x","y"])
##    #print("pc=",pc)
...
#    print("\nyc=\n",yc)
    ...
    Xr_df.drop(columns=["qty","last_order_upspd","prod_scaler","bin_no"],inplace=True)
...
# turns counter dictionary into a numpy array
 #   pc=pd.DataFrame(np.array(list(pcode_counts.items())),columns=["x","y"])
  ...
  #  plt.title("Customer code frequency as a count of total transactions")
    ...
    # plt.ylabel("Frequency")
    # plt.title("Sales Rep frequency as a count of total transactions")
    # plt.show()
...
#    print(dbd.columns)
    ...
    dbd["productgroup"]=fulldataset["productgroup"]
