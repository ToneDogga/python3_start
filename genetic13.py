...
class gene_string(object):
 ...
 #                       p=self.find_a_payoff2(val,payoff_filename)
              ...
              #  input("?")
               ...
               # print("\np=",p," val=",val)
                if direction=="x":  # maximising payoff
                    if p>max_payoff:
                         #if ((self.pconstrain and p>self.maxp) or (self.aconstrain and a>self.maxa) or (self.bconstrain and b>self.maxb) or (self.cconstrain and c>self.maxc) or (self.dconstrain and d>self.maxd) or (self.econstrain and e>self.maxe)):
                         #   pass
                         #else:
                            if not (self.pconstrain or self.aconstrain or self.bconstrain or self.cconstrain or self.dconstrain or self.econstrain):
                                best=dna[count]
                                bestrow=row_number     
                                max_payoff=p
                                found=True
       ...
       # input("?")
        if direction=="x":
            return(found,bestrow,best,max_payoff)
        elif direction=="n":
            return(found,bestrow,best,min_payoff)
        else:
...
#    input("?")
 ...
 #       print("total number of bits=",totalbits)
   ...
   #    print("number of mutations needed=",number_of_mutations_needed)
    ...
    def genetic_algorithm(self,pconstrain,minp,maxp,starta,finisha,aconstrain,maxa,mina,\
                      startb,finishb,bconstrain,maxb,minb,\
                      startc,finishc,cconstrain,maxc,minc,\
                      startd,finishd,dconstrain,maxd,mind,\
                      starte,finishe,econstrain,maxe,mine):

    length=20   #16   # 16 length of dna bit strings
    max_payoff=-10000
    min_cost=10000
    ...
    while extinctions<=extinction_events:
     ...
     #   clock_start=time.process_time()
            ...
            found,bestrow,fittest,returned_payoff=self.calc_fitness(dna,direction)
     ...
     #   print("calc fitness - Clock: duration_clock =", duration_clock)
                ...
                #print("returned payoff:",returned_payoff)
                    if direction=="x":
                        if returned_payoff>max_payoff:
                            ...
                            print("best fittest=",best," value=",int(best,2)," row number=",int(bestrow)," a=",axis[1]," b=",axis[2]," c=",axis[3]," d=",axis[4]," e=",axis[5],"generation no:",gen,"max_payoff=",max_payoff,flush=True)
                            outfile.write("best fittest= "+best+" value= "+str(int(best,2))+" row number= "+str(int(bestrow))+" a="+str(axis[1])+" b="+str(axis[2])+" c="+str(axis[3])+" d="+str(axis[4])+" e="+str(axis[5])+" generation no: "+str(gen)+" max_payoff= "+str(max_payoff)+"\n")
      ...
      #  clock_start=time.clock()
        ...
        #print(crossed)
            ...
            dna,mutation_count,gene_pool_size=self.mutate(crossed,length,mutation_rate)   # 1000 means mutation 1 in a 1000 bits processed
      ...
      #  print("mutate - Clock: duration_clock =", duration_clock)
        ...
        outfile.write("Finished - Clock: duration_clock ="+str(duration_clock)+"\n")
        outfile.close()
        print("")
...
if answer=="y":
    ...
    total_rows=generate_payoff_environment_5d_file(linewidth,0,2**4,0,2**4,0,2**4,0,2**4,0,2**4,payoff_filename)  
    clock_end=time.process_time()
    duration_clock=clock_end-clock_start
...
outfile.write("\n\n")
if xgene.pconstrain:
    outfile.write(str(xgene.minp)+" <= payoff/cost <= "+str(xgene.maxp)+"\n")

if xgene.aconstrain:
    outfile.write(str(xgene.mina)+" <= a <= "+str(xgene.maxa)+"\n")

if xgene.bconstrain:
    outfile.write(str(xgene.minb)+" <= b <= "+str(xgene.maxb)+"\n")

if xgene.cconstrain:
    outfile.write(str(xgene.minc)+" <= c <= "+str(xgene.maxc)+"\n")

if xgene.dconstrain:
    outfile.write(str(xgene.mind)+" <= d <= "+str(xgene.maxd)+"\n")

if xgene.econstrain:
    outfile.write(str(xgene.mine)+" <= e <= "+str(xgene.maxe)+"\n\n\n")
...
# 20 bits.  loop through every combination
for a in range(0,2**4):
    for b in range(0,2**4)
        for c in range(0,2**4)
            for d in range(0,2**4)
                for e in range(0,2**4)
                    xgene.genetic_algorithm(a,aconstrain,maxa,mina,\
                                              b,bconstrain,maxb,minb,\
                                              c,cconstrain,maxc,minc,\
                                              d,dconstrain,maxd,mind,\
                                              e,econstrain,maxe,mine):
