...
def return_a_row_from_file(val,filename):
        ...
        try:
            with open(filename,"r") as f:
                f.seek((LINEWIDTH+EXTRA_EOL_CHAR)*val)   # if a windows machine add an extra char for the '\r' EOL char       
          ...
          #      val=int(fittest,2)   # binary base turned into integer
                ...
                try:
                    ...
                    if r.row_find_method=="c":
                        ...
                        plist=return_a_row_from_file(int(fittest,2),r.payoff_filename)
                        plist[8]=plist[8].rstrip()
