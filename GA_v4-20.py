...
def return_a_row_from_csv3(Value=None):
        # we know that the each line in the file is exactly global "linewidth" bytes long including the '\n'
        try:

            with open("payoff_7d.csv", "rb") as csvfile:
                datareader = csv.reader(csvfile)
                #yield next(datareader)  # yield the header row
                count = 0
                for row in datareader:
...
def calc_mating_probabilities(newpopulation,r):
        count=0
       ...
       # print("new population size=",size)
        for gene in range(0,r.pop_size):
           # print("gene=",gene,"/",pop_size-1)
            ...
            if r.row_find_method=="l":
                total_payoff+=abs(find_a_payoff(val,r.payoff_filename))
               
            elif r.row_find_method=="s":    
                total_payoff+=abs(find_a_payoff2(val,r.payoff_filename))

            elif r.row_find_method=="r": 
 #               total_payoff+=abs(find_a_payoff3(val,payoff_filename))
                 #generator_p=find_a_payoff3(val,payoff_filename)
                 generator_payoff=return_a_row_from_csv3(val) 
                # p=generator_p.send(val)
                 p=generator_payoff.next()                   
                 print("p=",p)
                 total_payoff+=abs(p)
                 generator_payoff.close()
            else:
...
#    input("?")
     ...
     #   print("\ntotal payoff=",total_payoff)
        for gene in range(0,r.pop_size):
            ...
            if r.row_find_method=="l":   
                  p=abs(find_a_payoff(val,r.payoff_filename))
            elif r.row_find_method=="s":   
                  p=abs(find_a_payoff2(val,r.payoff_filename))
            elif r.row_find_method=="r":   
                  p=abs(find_a_payoff3(val,r.payoff_filename))
            else:
                print("row find method error.")
                sys.exit()
                
        #    if direction=="x":   # maximise
            probability_table.append(int(round((p/total_payoff)*r.scaling))) # scaling usually > pop_size*20
...
def genetic_algorithm(r):   # class r contains all the settings for the algorithm run
           ...
           # best_copy=False   # copy the population flag is set to true if the generation contains the highest fitness found so far
            ...
            for gene in range(0,size):  #newpopulation.iterrows():
                ...
                if val <= r.total_rows:   #total_rows:
                        try:
                            if r.row_find_method=="l":  # linecache
                                plist=return_a_row_as_a_list(val,r.payoff_filename)
                                #found=True
                        
                            elif r.row_find_method=="s":    
 #                             p=self.find_a_payoff2(val,payoff_filename)
                                plist=return_a_row_as_a_list2(val,r.payoff_filename)
                             #   found=True
...
#                    print("\rTotal Progress:[%d%%]" % (round(((total_generations+1)/(r.epoch_length*r.no_of_epochs))*100)),end="\r",flush=True)
                ...
                r.outfile.write("Epoch "+str(epoch)+"/"+str(r.no_of_epochs)+" Gen:[%d%%] " % (generation/r.epoch_length*100)+" generation # "+str(generation)+" fittest of this generation "+fittest+" row "+str(rowno)+"="+str(returned_payoff)+" best="+str(max_fittest)+"\n")
...
#                   print("\n\nFittest so far:",min_fittest,"best epoch:[",best_epoch,"] best generation in best epoch [",best_gen,"] best rowno [",best_rowno,"] min cost",min_payoff)
                ...
                r.outfile.write("Epoch "+str(epoch)+"/"+str(r.no_of_epochs)+" Gen:[%d%%] " % (generation/r.epoch_length*100)+" generation # "+str(generation)+" fittest of this generation "+fittest+" row "+str(rowno)+"="+str(returned_payoff)+" best="+str(min_fittest)+"\n")
       ...
       #             r.outfile.write("Genepool size "+str(pop_size)+" * min_scaling factor "+str(min_scaling)+" / scaling factor "+str(scaling_factor)+" = actual scaling: "+str((pop_size*min_scaling)/scaling_factor)+"\n")
         ...
         #       r.outfile.write("Fittest so far "+str(best_fittest)+" best epoch "+str(best_epoch)+" best generation in best epoch "+str(best_gen)+" best row no "+str(best_rowno)+" min cost "+str(min_payoff)+"\n")
            ...
            probability_table=calc_mating_probabilities(population,r)
...
def main():
    ...
    ga.number_of_cols=9   # 1xrowno, 7 input vars and 1 out =9
...
###############################################################
        ...
        genetic_algorithm(ga)      #  call the algorithm for each setting
