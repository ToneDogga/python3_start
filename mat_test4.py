...
@tf.function
def create_X_and_Y_batch(series,batch_size):
  #  print("s",series,series.shape)
 #   begin=tf.constant([0,810],dtype=tf.int32)
 #   batch_size=tf.constant([1,batch_size],dtype=tf.int32)
 #   print(begin,batch_size)
    unique_batches=series.shape[1]-batch_size 
    
    X_batches=[tf.stack((series[0,p:p+batch_size])) for p in tf.range(unique_batches)]  #.to_numpy()
    tf.print("XB",X_batches,len(X_batches))   #,X_batches.shape)

    dataset=tf.data.Dataset.from_tensor_slices([X_batches,X_batches])

 #   dataset=dataset.map(preprocess,num_parallel_calls=None)
    
   # tf.print("list dataset",list(dataset.as_numpy_iterator()))
...
#        dataset = tf.data.Dataset.from_tensor_slices(plot_dict[k])
        ...
        train_set=create_X_and_Y_batch(plot_dict[k],batch_size)
  #      valid_set=create_X_and_Y_batch(dataset,batch_size)
  #      test_set=create_X_and_Y_batch(dataset,batch_size)

 
        for _ in range(10):
         #   tf.print(iterator.get_next())  #,i[0].shape)      
            tf.print(train_set)   #.get_next())
