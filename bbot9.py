...
#############################################
   #  key constants
    ...
    deadzone_maxangle=2
    tipped_over_angle=26  # plus or minus 20 degrees.  Kill the motor and let the robot stop
    ...
    IKp=9       # (9)proportional constant    
    IKi=0.19  # (0.18)integral constant ( frequency=23 x 10)=230, period of occilation is approx 150. or frequency is 8x10=80, half of occilation
    IKd=0.18    # (0.18)differential constant 

    # outer loop PID constants  speed error, adjust the target angle
    OKp=18      # proportional constant 
    OKi=1     # integral constant(x10) 0.0005 
    OKd=2       # differential constant 
   ...
   # emergency_power_percent=25
    ...
    #BP.set_motor_limits(BP.PORT_B+BP.PORT_C, normal_power_percent, max_speed_limit)
   # BP.set_motor_limits(BP.PORT_B+BP.PORT_C, 100, 5000)
...
# is the robot tipped over?
          ...
          #      robotlog.write("TIPPED OVER. time="+str(timelog)+" actual angle="+str(actual_angle)+" relative angle "+str(relative_angle)+", rate:"+str(angle_rate)+" p="+str(p)+" i="+str(i)+" d="+str(d)+" PID motor speed="+str(motor_speed)+" Kp="+str(Inner_PID.Kp)+" Ki="+str(Inner_PID.Ki)+" Kd="+str(Inner_PID.Kd)+"\n")
                ...
                p,i,d,i_error=Inner_PID.PID_processor(actual_angle,target_angle,angle_rate,IKp,IKi,IKd,10,8)   # 12, 5 current angle, command angle and gyro rate (as the differential element), integral number of samples, and sample rate
