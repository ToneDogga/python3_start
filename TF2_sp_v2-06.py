...
#    y_length=1
    ...
    mats=[21]   #omving average window periods for each data column to add to series table
    start_point=np.max(mats)+1  #batch_length+1   #np.max(mats) #+1
    mat_types=["u"]  #,"d","m"]
...
#            if (n_inputs>2) & ((layer_count==3) | (layer_count==5)):  # | (layer_count==5) | (layer_count==6):
            if ((layer_count==3) | (layer_count==5)):  # | (layer_count==5) | (layer_count==6):
