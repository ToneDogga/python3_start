...
#        cbit_mask3="condition==\""
        ...
        for elem in bits:
           ...
           #     bit_question.append(input(name+" bit:"+number+" y/n/#?"))
                notcorrect=True
                while notcorrect:
                    bbinput=input(name+" bit:("+number+") 1/0/?")
                    notcorrect=not (bbinput=="1" or bbinput=="0" or bbinput=="?")
                    if notcorrect:
                        print("Please enter either 1 or 0 or ?")
 ...
 #               bit_mask3="message==\""
        ...
        loop=not (("?" in bit_question) and (("1" in bit_question) or ("0" in bit_question)))
        if loop:
            print("Question must have at least one '?' and at least one '1' or '0', please reenter.")
   ...
   # import_file="metaresults_csv.csv"
    ...
    if len(bits)==0:
       ...
       # print("First condition:",conditionstr,"first message:",messagestr)
        savefile=input("Enter decode bits save filename.  Saved as a .dat:")
