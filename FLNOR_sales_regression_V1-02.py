...
def cleanout_slow_sellers(df,n):
    ...
    count=Counter(df["prod_encode"])
    print("n=",n,"=>,",count)
    lowtrans=list(count.most_common()[:-n-1:-1])
...
def order_delta(df):
    df.sort_values(by=["prod_encode","day_delta"],ascending=[True,False],inplace=True)
##
 ...
 #   df["previous_qty"]=df["qty"].shift(periods=-1,fill_value=0.0)
  ...
  #  df["last_week_order_qty"]=df.query("day_order_delta<=7.0").qty.shift(periods=-1,fill_value=11.11)
    ...
    prod_list=list(set(df["prod_encode"].tolist()))

    for prod in prod_list:
    #    last_order=((df["prod_encode"]==prod) & (df2.day_order_delta>=4) & (df2.day_order_delta<=45))
        last_order=((df["prod_encode"]==prod))    # & (df2.day_order_delta>=4) & (df2.day_order_delta<=45))

        df["last_order_qty"]=df[last_order].qty.shift(periods=-1,fill_value=0)
        df1.loq=df2[last_order].qty.shift(periods=-1,fill_value=0)
        df2.loc[df1.loq.index,"last_order_qty"]=df1.loq
     #   df1["loq"]=df2[last_order].qty.shift(periods=-1,fill_value=0)
     #   df2.loc[df1["loq"].index,"last_order_qty"]=df1["loq"]

        df2.drop(df2[(df2["day_order_delta"]==0)].index,inplace=True)
        df2.loc[df1.loq.index,"last_order_upspd"]=df1.loq/(df2.day_order_delta+0.001)
        df2.drop(df2[(df2["last_order_qty"] == 0)].index,inplace=True)
        
   #    print(prod,"1=",df2[last_order])
        
     #   df2.day_order_delta.clip_lower(0,inplace=True)
    #    df2.drop(df2.loc[df2['last_order_qty'] == 0].index,inplace=True)
   #  df.drop(columns=["code","product"],inplace=True)

    #    print(prod,"2=",df2[last_order])
##
##    for prod in prod_list:
...
##################################################33
   # Remove extranous fields
    ...
    print("Delete rows for productgroup <10 or >16.")
    b4=X_df.shape[0]
    X_df.drop(X_df[(X_df["productgroup"]<10) | (X_df["productgroup"]>16)].index,inplace=True)
 ...
 #   joblib.dump(label_encoder,open(glset_encode_save,"wb"))
    ...
    #X_df = X_df[["prod_encode","qty","date","date_encode","day_delta","day_of_year","week_of_year","month_of_year","year"]]  
    X_df = X_df[["prod_encode","qty","productgroup","date","date_encode","day_delta","day_of_year","week_of_year","month_of_year","year"]]  
    print(X_df.columns)
...
##    print(l)   #ist(set(X_df.date.tolist())).sort()
    ...
    X_df=cleanout_slow_sellers(X_df,16)   # any product with 16 or less transactions in the dataset is removed here

##    label_encoder=LabelEncoder()
##    #label_encoder=OrdinalEncoder()
##
##    #one_hot_encoder=OneHotEncoder()
## #   X_df = label_encoder.fit_transform(X_df.to_numpy())
##
##    #label_encoder.fit(X_df)    #["product"])
##    
##    X_df["prod_encode"] = label_encoder.fit_transform(X_df["product"].to_numpy())
##    X_df["last_weeks_prod_encode"] = label_encoder.fit_transform(X_df["last_weeks_product"].to_numpy())
##    X_df["last_two_weeks_prod_encode"] = label_encoder.fit_transform(X_df["last_two_weeks_product"].to_numpy())
##    X_df["last_month_prod_encode"] = label_encoder.fit_transform(X_df["last_month_product"].to_numpy())
##    X_df["last_year_prod_encode"] = label_encoder.fit_transform(X_df["last_year_product"].to_numpy())
##
##    joblib.dump(label_encoder,open(product_encode_save,"wb"))
 ...
 #   print(X_df)
  ...
  #  X_df.drop(columns=["last_weeks_product","last_two_weeks_product","last_month_product","qty"],inplace=True)
    X_df.drop(columns=["qty"],inplace=True)

  #  X_df=X_df[columns=[".astype(int)

    counts=Counter(X_df.productgroup)   #.unique()
    print("\nFrequency of product groups:",counts)   #dict(zip(unique, counts)))
    X_df.drop(columns=["productgroup"],inplace=True)
...
##        print('Score with the data set of top {0} features: {1:.2f}'.format(k, score_k_features))
  ...
  #  print("Xr_test=\n",Xr_test)
    ...
    j=pd.DataFrame(Xr_test,columns=["prod_encode","date_encode",])
  ...
  #  print("j=\n",j)
    ...
    k=pd.DataFrame(np.hstack((j,predictions.reshape(-1,1))),columns=["date","product","predict_qty"])
...
#  dt.datetime.today().strftime("%d/%m/%Y")
 ...
 #   print("k=\n",k[0:15].to_string())
    ...
    k= k[["date","product","predict_qty"]]  
    k.sort_values(by=["date","product"],axis=0,ascending=[False,True],inplace=True)
