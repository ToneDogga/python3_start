...
class QueryWindow(pyglet.window.Window):
    ...
    def on_key_press(self, symbol, modifiers):
        ir.act_on_key_state(self.x_temp,self.y_temp,symbol,modifiers)
        # if symbol == key.F1:
...
window = QueryWindow(1520,900,resizable=False,visible=False,caption="Query on "+str(load_dir)+str(load_file),config=config)
#xloc, yloc = window.get_location()
#window.set_location(xloc + 300, yloc)
  ...
  #  sprite.draw()
    ...
    def display_number_of_records(self,sales_df,query_dict,query_df_list,x,y):
     ...
     #   pyglet.TextLayout.color=(255,255,255,255) 
        ir.position_text_in_active_window("To Select:Mouse over the fields rect, then PageUp/Pagedown/up/down arrows/mouse scroll wheel or type first letter. Press F1-add a selection, F2-remove the selected val. Esc to exit.",x=x,y=y)
      ...
      #  if len(query_df_list)>0:
        ...
        if len(query_df_list)==1:
            ...
            if self.qdf_size!=qdf.shape[0]: 
                ...
                self.mat_sprite.y=10
                self.mat_sprite.scale_y=0.43
                self.mat_sprite.scale_x=0.34
                ...
                self.pareto_customer_sprite.y=10
                self.pareto_customer_sprite.scale_y=0.43
                ...
                self.pareto_product_sprite.y=10
...
#            position_text_in_active_window("Query="+str(self.query_dict_save),x=x,y=y-65)
            ...
            if isinstance(start_date,pd.Timestamp) & isinstance(end_date,pd.Timestamp):  
                ...
                ir.position_text_in_active_window("Query day length="+str(query_span)+" days",x=x,y=y-115)
...
class interactive_driver(object):
    def __init__(self):
     
        global query_df_list 
        query_df_list=[]
        global query_dict
        query_dict={}
    #    query_df_list
    ...
    def act_on_key_state(self,x,y,symbol,modifiers):
        #keys = key.KeyStateHandler()
        #window.push_handlers(keys)
    ...
    def check_for_mouse_over(self,x,y):
    ...
    def draw_buttons(self):
        ...
        for b in BUTTON_LIST:
            if b.visible:
                batch=self._draw_button(b,batch)
    ...
    def _draw_button(self,button,batch):       
        if button.active:
    ...
    def draw_pointers(self,x,y,x_max,y_max):
        batch = pyglet.graphics.Batch()
     #   batch.add(2, pyglet.gl.GL_LINES, None,
      #                           ('v2i', (0, 0, x, y,0,y_max)),
                             #    ('v2i', (x_max,0, x, y,x_max,y_max)), 
    ...
    def position_text_in_active_window(self,text,*,x,y):
        batch = pyglet.graphics.Batch()
       # canvas={}
    ...
    def position_list_in_active_window(self,*,x,y,input_list,selection_list_input):
        batch = pyglet.graphics.Batch()
    ...
    def move_and_draw_pointer(self,x,y):
       ...
       #     batch = pyglet.graphics.Batch()
         ...
         #   st.display_number_of_records(sales_df,query_dict,query_df_list)
            if MY_DEBUG:
                self.draw_pointers(x,y,window.x_max,window.y_max)
            self.check_for_mouse_over(x,y)
    ...
    def interactive_query_window(self):
        window.set_visible()
