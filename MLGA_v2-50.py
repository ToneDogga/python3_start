...
##    
##
#def calculate_crowding_factor(population,params):
 #   testlist = population["condition"].astype('category')
   # pd.cut(population.value, range(0, 105, 10), right=False, labels=labels)

    
##    similar_matrix=[]
##    for i in range(0,len(population)):
##        condition=population.iloc[i].condition
##        similar_matrix.append(population.apply(similarity,axis=1,args=[condition]).tolist())
##    print("sm=",similar_matrix)
##    input("?")
##    return(population)
   ...
   #     print(population.dtypes)
    #    population = pd.DataFrame("match_flag", dtype=np.bool)
     #   population["match_flag"].dtype=np.bool
        ...
        individual = np.dtype([("condition",condition_len),("message",message_len),("bid",'f'),("ebid",'f'),("match_flag",'b'),("strength",'f'),("specificity",'f'),("crowding",'f'),("payments","f"),("taxes","f"),("receipts","f"),("winners","b")])
        poparray = np.zeros(p["size"], dtype=individual) 
        population = pd.DataFrame({"condition":poparray["condition"],"message":poparray["message"],"bid":poparray["bid"],"ebid":poparray["ebid"],"match_flag":poparray["match_flag"],"strength":poparray["strength"],"specificity":poparray["specificity"],"crowding":poparray["crowding"],"payments":poparray["payments"],"taxes":poparray["taxes"],"receipts":poparray["receipts"],"winners":poparray["winners"]})  #,"xpoint":population['xpoint'],"chromopack":population['chromopack'],"expressed":population['expressed']})   #,'area': areaprint("\n\n")
...
##            print("m=",m)
    ...
    while count<params["epoch"]:   # loop while the message list is not empty
     ...
     #       params["message_list"].insert(0,multiplexer(message_list,params["mplx_data"]))   #   send a message to the multiplexer, process the output as a message
      ...
      #  population=calculate_crowding_factor(population,params)   # the similarity of each condition to the rest of the population
        # this is used to target a condition to die
...
#
    params = dict(
       ...
       # input_file=sys.argv[1],
        ...
        bidtax=0.01,
        lifetax=0.0,
        reward=1.08,    # 1.45 reward scaling factor from payment
        ...
        mutrate=17,  #[0,1,2],  every 50 cycles
        birthrate=7,  # 2 new conditions are born every ? cycles
        deathrate=13,  # an old unfit condition or duplicate is killed every ? cycles
