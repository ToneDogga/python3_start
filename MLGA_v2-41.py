...
def decode_signal(signal,no_of_data_bits,no_of_address_bits):   # multiplexer in form   ddddaa -> o
  ...
  #  print("data=",data)
    result=data[address]
  #  print("signal=",signal,"address=",address,"data=",data,"result=",result,"\n")
   # input("?")
    return(result)
  ...
  #      input("?")
      ...
      #      print("death len after=",len(population),"pop after=",population)
       ...
       #     print("birth len after=",len(population),"pop after=",population)
        ...
        df=population.drop_duplicates(subset=["condition"])   # remove non-unique conditions
        population=df
...
def main():
    ...
    params = dict(
       ...
       # input_file=sys.argv[1],
        ...
        bidtax=0.04,
        lifetax=0.007,
        reward=1.55,    # 1.45 reward scaling factor from payment
        ...
        mutrate=200,  #[0,1,2],  every 50 cycles
        birthrate=4,  # 2 new conditions are born every ? cycles
        deathrate=8,  # an old unfit condition is killed every ? cycles
        condition_match_specificity=0.7,   #  the condition needs only to match the signal 70% of the alleles in the gene to be marked a match
