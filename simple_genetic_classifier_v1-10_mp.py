...
def create_entire_classifier_space(population,params):
    ...
    for c_allele in range(0,3**params["condition_bits"]):
        if pos%100==0:
        ...
        for mcount in range(0,2**params["message_bits"]):
...
# delete a non matching, non winner at random
  ...
  #  population.drop(population['strength'].idxmin())
    mean=population["strength"].mean()
    started=params["starting_strength"]
   ...
   #     print(population.dtypes)
    #    population = pd.DataFrame("match_flag", dtype=np.bool)
     #   population["match_flag"].dtype=np.bool
        ...
        poparray = np.zeros(1, dtype=individual) 
...
def classifier_mp_GA(params):   # multiprocessing stuff will go here later
    ...
    print("Parameters=\n",params)
    print("\n\nPlease wait. Creating",params["size"],"unique (or non-unique) classifiers.")
 ...
 #   print("Multiprocessing starting. cpus=",cpus)
    ...
    with Pool(processes=cpus) as pool:  # processes=cpus-1
        ...
        add_to_queue(pickle.dumps("Generate results finished\n"),q)
...
##        no_of_data_bits=2**2,   # binary,  in this case 6 input , 1 output
        ...
        create_unique_classifiers_flag=True, #False, #True,   # if true it creates all possible classifiers (conditions : messages) so that they are all unique.  It ignores the size number below, if false they are selected randomly to the size number below
       ...
       # input_file=sys.argv[1],
        ...
        bidcoeff2=0.06,    #  added and adjusted by specificity of the genes ie the amountof '#'.  The total of the two should equal approx 0.12
        bidsigma=0.03,
        bidtax=0.0,   #0.01
        lifetax=0.0,
