         ...
         #   print("Epochs:",epoch+1,"Generation progress: [%d%%]" % (generation/epoch_length*100),"diversity (len_sel)=",len_sel,"Tot gene bits:%d" % total_genes,"Tot Mutations:%d      "  % (mutations), end='\r', flush=True)
          ...
          #  bestpopulation=population
           # print("len genepool=",size)
            for gene in range(0,size-1):  #newpopulation.iterrows():
                plist=[]
                val=int(population.loc[gene,"expressed"],2)   # binary base turned into integer
                  #  print("val=",val)
                if val <= size:
                        try:
                            if row_find_method=="l":  # linecache
 ...
 #                             p=self.find_a_payoff2(val,payoff_filename)
     ...
     #       print(population)
      #      input("?")
            ...
            print("\rEpoch no:",epoch,"/",no_of_epochs,"  Gen progress: [%d%%]" % (generation/epoch_length*100)," dvrsty=",len_sel," fittest=",fittest," rowno=",rowno," curr poff",returned_payoff," best=",max_fittest,"bgen=",best_gen,"bestrno=",best_rowno," max_poff=",max_payoff,"         ",end='\r',flush=True)
