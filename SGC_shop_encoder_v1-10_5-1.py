...
# create pandas outfile dataframe
    envdata_out_type = np.dtype([(col[0],"S"+str(bits[0])),(col[1],"S"+str(bits[1])),(col[2],"S"+str(bits[2])),(col[3],"S"+str(bits[3])),(col[4],"S"+str(bits[4])),(col[5],"S"+str(bits[5]))])
    envdata_array=np.zeros(endlen,dtype=envdata_out_type)
    envdata_out=pd.DataFrame({col[0]:envdata_array[col[0]],col[1]:envdata_array[col[1]],col[2]:envdata_array[col[2]],col[3]:envdata_array[col[3]],col[4]:envdata_array[col[4]],col[5]:envdata_array[col[5]]})
...
##############################################
  ...
  #  envdata_out2=envdata_out.assign(condition =lambda x: (x[col[0]]+x[col[1]]+x[col[2]]+x[col[3]]+x[col[4]]+x[col[5]]+x[col[6]]))
   # envdata_out2["condition"]=envdata_out.apply(lambda x: (x[col[0]]+x[col[1]]+x[col[2]]+x[col[3]]+x[col[4]]+x[col[5]]+x[col[6]]), axis=0)
    envdata_out2["condition"]=envdata_out[col[0]]+envdata_out[col[1]]+envdata_out[col[2]]+envdata_out[col[3]]+envdata_out[col[4]]

   # envend=len(envdata_out.columns)
   
 #   envdata_out2["message"]=envdata_out.apply(lambda x: (x[col[7]]))
    envdata_out2["message"]=envdata_out[col[5]]
...
#
    params = dict(
       ...
       # epoch=int(sys.argv[1]),  #1000,
        infile="shopsales31_5-1.csv",   #MLGA_environment.csv",
        outfile="shop_SGC_encoded31_5-1.csv",   #MLGA_environment.csv",
        concatfile="shop_concat_SGC_encoded31_5-1.csv",
        reportfile="SGC_encoded_report_5-1.txt")
