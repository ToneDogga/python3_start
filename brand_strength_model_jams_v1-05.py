...
############################################3
  ...
  #  X_df['day_delta'] = (X_df.date-X_df.date.min()).dt.days.astype(int)
    ...
    X_df.drop(columns=["date","date_encode","coles_scan_week","bb_price","sd_price","c_price","bm_price","bb_promo_off","sd_promo_off","c_promo_off","bm_promo_off"],inplace=True)
...
##    X_df["bm_upw_total"]=(X_df["bm_upspw_baseline"]+X_df["bm_upspw_incremental"])*X_df["bm_dd"]
    ...
    X_df[["day_delta","bb_upspw_baseline","sd_upspw_baseline","c_upspw_baseline","bm_upspw_baseline","bb_upspw_incremental","sd_upspw_incremental","c_upspw_incremental","bm_upspw_incremental","bb_dd","sd_dd","c_dd","bm_dd","bb_on_promo","sd_on_promo","c_on_promo","bm_on_promo","bb_promo_disc","sd_promo_disc","c_promo_disc","bm_promo_disc","bb_total_units","sd_total_units","c_total_units","bm_total_units"]]
...
######################################################################
   ...
   #sdsalestable= pd.pivot_table(X_df, values='sd_total_upspw', index=['day_delta'],columns=['sd_total_upspw'], aggfunc=np.sum, margins=True, fill_value=0)
    sdsales=X_df["sd_total_units"].mean()   
    print("WW SD sales total upw mean=\n",sdsales)
    bmsales=X_df["bm_total_units"].mean()   
    print("WW BM sales total upw mean=\n",bmsales)
    bbsales=X_df["bb_total_units"].mean()   
    print("WW BB sales total upw mean=\n",bbsales)
    csales=X_df["c_total_units"].mean()   
    print("WW C sales upw total mean=\n",csales)


##########################################

 #   important_attributes=["bb_total_upspw","sd_total_upspw","bm_total_upspw","c_total_upspw"]
  #  scatter_matrix(X_test[important_attribto_datetimeutes],alpha=0.2,figsize=(12,9))
...
############################################3
    ...
    y2=X_df["bb_total_units"].to_numpy()  # regression

    X_baseline=X_df["bb_upspw_baseline"].to_numpy()  # regression
    X_total=X_df["bb_total_units"].to_numpy()  # regression
 
    
    X_df.drop(columns=["bb_upspw_baseline","bb_total_units"],inplace=True)
...
##
##        regressor_best = grid_search.best_estimator_
##       # regressor_best.score(Xr_test, yrto_datetime_test)
##
##        predictions = regressor_best.predict(Xr_scaled_test)
##
...
#############################################################################################3
        ...
        dbd=pd.DataFrame(np.copy(X_test),columns=["bb_dd","bb_on_promo","bb_promo_disc","bb_promo_price","bb_upspw_incremental","sd_dd","sd_on_promo","sd_promo_disc","sd_promo_price","sd_upspw_baseline","sd_upspw_incremental","sd_total_units","c_dd","c_on_promo","c_promo_disc","c_promo_price","c_upspw_baseline","c_upspw_incremental","c_total_units","bm_dd","bm_on_promo","bm_promo_disc","bm_promo_price","bm_upspw_baseline","bm_upspw_incremental","bm_total_units","day_delta"])
        dbd["bb_upspw_baseline"]=y_test
        dbd["bb_total_units"]=y2_test
      
        dbd["predict_bb_upspw_baseline"]=predictions.reshape(-1,1)
        dbd["predict_bb_total_units"]=predictions2.reshape(-1,1)

       # dbd2=pd.DataFrame(np.hstack((dbd.to_numpy(),predictions.reshape(-1,1))),columns=["day_order_delta","code","product","date","predict_qty"])


        scatter_matrix(dbd[["bb_upspw_baseline","predict_bb_upspw_baseline","bb_total_units","predict_bb_total_units"]],alpha=0.2,figsize=(12,9))
...
#####################################################################################333 
    ...
    dbe=pd.DataFrame(np.copy(X_df),columns=["bb_dd","bb_on_promo","bb_promo_disc","bb_promo_price","bb_upspw_incremental","sd_dd","sd_on_promo","sd_promo_disc","sd_promo_price","sd_upspw_baseline","sd_upspw_incremental","sd_total_units","c_dd","c_on_promo","c_promo_disc","c_promo_price","c_upspw_baseline","c_upspw_incremental","c_total_units","bm_dd","bm_on_promo","bm_promo_disc","bm_promo_price","bm_upspw_baseline","bm_upspw_incremental","bm_total_units","day_delta"])    #["bb_dd","sd_dd","c_dd","bm_dd","sd_upspw_baseline","c_upspw_baseline","bm_upspw_baseline","bb_on_promo","sd_on_promo","c_on_promo","bm_on_promo","bb_promo_disc","sd_promo_disc","c_promo_disc","bm_promo_disc","bb_promo_price","sd_promo_price","c_promo_price","bm_promo_price","bb_upspw_incremental","sd_upspw_incremental","c_upspw_incremental","bm_upspw_incremental","sd_total_units","c_total_units","bm_total_units","day_delta"])
    print(dbe.shape)
    dbe["date"]=X_date
    dbe["bb_upspw_baseline"]=X_baseline
    dbe["bb_total_units"]=X_total

    print("dbe=\n",dbe.columns,"\n\n",dbe.shape)

    sf=input("display scatter graphs? (y/n)")
    sct_flag=(sf=="y")
    if sct_flag:

    
##        scatter_matrix(dbe[["bb_upspw_baseline","sd_promo_disc","bm_promo_disc","c_promo_disc"]],alpha=0.2,figsize=(12,9))
##        plt.show()
##
##
##        scatter_matrix(dbe[["sd_upspw_baseline","bb_promo_disc","bm_promo_disc","c_promo_disc"]],alpha=0.2,figsize=(12,9))
##        plt.show()
##
##
##        scatter_matrix(dbe[["bm_upspw_baseline","bb_promo_disc","sd_promo_disc","c_promo_disc"]],alpha=0.2,figsize=(12,9))
##        plt.show()
##
##
##        scatter_matrix(dbe[["c_upspw_baseline","bb_promo_disc","sd_promo_disc","bm_promo_disc"]],alpha=0.2,figsize=(12,9))
##        plt.show()


        scatter_matrix(dbe[["bb_total_units","sd_promo_disc","bm_promo_disc","c_promo_disc"]],alpha=0.2,figsize=(12,9))
        plt.show()


        scatter_matrix(dbe[["sd_total_units","bb_promo_disc","bm_promo_disc","c_promo_disc"]],alpha=0.2,figsize=(12,9))
        plt.show()


        scatter_matrix(dbe[["bm_total_units","bb_promo_disc","sd_promo_disc","c_promo_disc"]],alpha=0.2,figsize=(12,9))
        plt.show()


        scatter_matrix(dbe[["c_total_units","bb_promo_disc","sd_promo_disc","bm_promo_disc"]],alpha=0.2,figsize=(12,9))
        plt.show()



        scatter_matrix(dbe[["bb_total_units","sd_promo_disc"]],alpha=0.2,figsize=(12,9))
        plt.show()


        scatter_matrix(dbe[["bb_total_units","bm_promo_disc"]],alpha=0.2,figsize=(12,9))
        plt.show()


        scatter_matrix(dbe[["bb_total_units","c_promo_disc"]],alpha=0.2,figsize=(12,9))
        plt.show()


        scatter_matrix(dbe[["sd_total_units","bm_promo_disc"]],alpha=0.2,figsize=(12,9))
        plt.show()


        scatter_matrix(dbe[["sd_total_units","c_promo_disc"]],alpha=0.2,figsize=(12,9))
        plt.show()


        scatter_matrix(dbe[["sd_total_units","bb_promo_disc"]],alpha=0.2,figsize=(12,9))
        plt.show()


        scatter_matrix(dbe[["bm_total_units","sd_promo_disc"]],alpha=0.2,figsize=(12,9))
        plt.show()


        scatter_matrix(dbe[["bm_total_units","c_promo_disc"]],alpha=0.2,figsize=(12,9))
        plt.show()


        scatter_matrix(dbe[["bm_total_units","bb_promo_disc"]],alpha=0.2,figsize=(12,9))
        plt.show()


        scatter_matrix(dbe[["c_total_units","sd_promo_disc"]],alpha=0.2,figsize=(12,9))
        plt.show()


        scatter_matrix(dbe[["c_total_units","bm_promo_disc"]],alpha=0.2,figsize=(12,9))
        plt.show()


        scatter_matrix(dbe[["c_total_units","bb_promo_disc"]],alpha=0.2,figsize=(12,9))
        plt.show()
...
##########################################################3    
    ...
    #outfile=".\\"+cfg.scalerdump2
    outfile=cfg.scalerdump2

    print(outfile)

    dbe.sort_values(by=["date"],axis=0,ascending=[True],inplace=True)
    dbf=dbe.copy(deep=True)

    dbf["date"]=pd.to_datetime(dbe["date"])
    startdate=dbf["date"].min()
    finishdate=dbf["date"].max()
    print("sd=",type(startdate),startdate,"fd=",type(finishdate),finishdate)
    start_row=0
    sheet_name_no=1
 #   book=load_workbook(file)
    current_finish_date=finishdate
    current_start_date=current_finish_date+relativedelta(weeks=-80)
   # ws=wb.active

    
    writer= pd.ExcelWriter(outfile,engine="openpyxl",mode='a') #as writer:  # mode="a" for append
    wb= load_workbook(outfile)
    writer.wb=wb
 #   workbook = writer.book
 #   worksheet = writer.sheets['report']

    while current_finish_date >= startdate and current_start_date >=startdate:
       
        mask=((dbf["date"]>=current_start_date) & (dbf["date"]<=current_finish_date))
        dbg=dbf[mask]
      #  print("dbg=\n",dbg)
    ...
    #    print("\n\nCorrelations:\n",current_start_date,"to",current_finish_date,":\n",corr_matrix)
        sname="c"+str(current_start_date.strftime('%Y%m%d'))+"-"+str(current_finish_date.strftime('%Y%m%d'))
       # print(sname,"\n\n")
        sdf=pd.DataFrame(columns=["Range","Type","Method"])
        
        sdf.at[0,"Range"]=sname
        sdf.at[0,"Type"]="Total upspw"
        sdf.at[0,"Method"]="Pearson R"

       # print("sdf=",sdf)

        sdf.to_excel(writer,sheet_name="Sheet1",startrow=start_row, index=True)
        sarraydf = pd.DataFrame (corr_matrix)
        sarraydf.to_excel(writer,sheet_name="Sheet1",startrow=start_row+2, index=True)

        start_row+=len(sarraydf.columns)+4


        
        important_attributes=["date","bb_upspw_baseline","sd_upspw_baseline","bm_upspw_baseline","c_upspw_baseline","bb_promo_disc","sd_promo_disc","bm_promo_disc","c_promo_disc"]
        dbh=dbg[important_attributes]
        corr_matrix2=dbh.corr(method="pearson")    #important_attributes)   #.sort_values(ascending=False)   #[important_attributes])   #important_attributes2,method="pearson")

     #   print("\n\nCorrelations:\n",current_start_date,"to",current_finish_date,":\n",corr_matrix2)
      #  sname="c"+str(current_start_date.strftime('%Y%m%d'))+"_"+str(current_finish_date.strftime('%Y%m%d'))
       # print(sname,"\n\n")
        sdf2=pd.DataFrame(columns=["Range","Type","Method"])
        
        sdf2.at[0,"Range"]=sname
        sdf2.at[0,"Type"]="Baseline upspw"
        sdf2.at[0,"Method"]="Pearson R"


       # print("sdf=",sdf)

        sdf2.to_excel(writer,sheet_name="Sheet1",startrow=start_row, index=True)
        sarraydf2 = pd.DataFrame (corr_matrix2)
        sarraydf2.to_excel(writer,sheet_name="Sheet1",startrow=start_row+2, index=True)

        start_row+=len(sarraydf2.columns)+4
        dbg=dbf[mask]
      #  print("dbg=\n",dbg)
        
        important_attributes=["date","bb_total_units","sd_total_units","bm_total_units","c_total_units","bb_promo_disc","sd_promo_disc","bm_promo_disc","c_promo_disc"]
        dbh=dbg[important_attributes]
        corr_matrix=dbh.corr(method="spearman")    #important_attributes)   #.sort_values(ascending=False)   #[important_attributes])   #important_attributes2,method="pearson")
    ...
    #    print("\n\nCorrelations:\n",current_start_date,"to",current_finish_date,":\n",corr_matrix)
       ...
       # print("sdf=",sdf)
        ...
        important_attributes=["date","bb_upspw_baseline","sd_upspw_baseline","bm_upspw_baseline","c_upspw_baseline","bb_promo_disc","sd_promo_disc","bm_promo_disc","c_promo_disc"]
        dbh=dbg[important_attributes]
        corr_matrix2=dbh.corr(method="spearman")    #important_attributes)   #.sort_values(ascending=False)   #[important_attributes])   #important_attributes2,method="pearson")
...
##        sarraydf2.to_excel(writer,sheet_name="Sheet2", startrow=start_row, index=True)


        current_start_date+=relativedelta(weeks=-13)
        current_finish_date+=relativedelta(weeks=-13)
        if current_start_date < startdate:
    ...
    writer.save()
    print("\n\nCorrelation matrix array saved to",cfg.scalerdump2)          
#
###################################################################
