...
##        input("?")
      #  print("len before=",len(population),"pop before=",population)
       ...
       #     population=population.drop(nan_rows)   #[population.isnull().any(1)]
        ...
        if death_flag and count>params["deathstart"]:
            pass
...
##            population=df2
      ...
      #      print("death len after=",len(population),"pop after=",population)
        ...
        if crowding_flag and count>params["crowdingstart"]:
        ...
        if birth_flag and count>params["birthstart"]:
          #  print("birth flag")
...
#
    params = dict(
       ...
       # input_file=sys.argv[1],
        ...
        bidcoeff2=0.04,    #  added and adjusted by specificity of the genes ie the amountof '#'.  The total of the two should equal approx 0.12
        bidsigma=0.03,
        bidtax=0.0,   #0.01
        lifetax=0.0,
        ...
        mutrate=99,  #[0,1,2],  every 50 cycles
        birthrate=87,  # 2 new conditions are born every ? cycles
        birthstart=1800,  # the number of cycles before repdocution starts
