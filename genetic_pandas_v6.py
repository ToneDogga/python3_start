...
#########################################
   ...
   # print(population)
    ...
    if answer=="y":
       ...
       # 7 input variables 2 bits each = 14 bits, 1 floating point payoff.
        total_rows=generate_payoff_environment_7d_file(linewidth,0,2**2,0,2**2,0,2**2,0,2**2,0,2**2,0,2**2,0,2**2,payoff_filename)
 ...
 #                             p=self.find_a_payoff2(val,payoff_filename)
                    ...
                    #        population.loc[val, "fitness"] = p
                          ...
                          #  best_rowno=rowno
                            #best_flag=False
                           # print("best row and gen found",best_gen,best_rowno)
     ...
     #       print(population)
      #      input("?")
            ...
            print("\rEpoch",epoch,"/",no_of_epochs,"Gen:[%d%%]" % (generation/epoch_length*100),"len_sel",len_sel,"fittest",fittest,"rowno",rowno,"poff",returned_payoff,"best",max_fittest,"be",best_epoch,"bgen",best_gen,"brno",best_rowno,"maxpoff",max_payoff,"        ",end='\r',flush=True)
       ...
       #print("returned payoff:",returned_payoff)
            ...
            wheel=calc_mating_probabilities(population,pop_size,direction,scaling_factor,row_find_method,payoff_filename)  # scaling figure is the last.  this multiplies the payoff up so that divsity is not lost on the wheel when probs are rounded
            if len(wheel)==0:
                    print("wheel empty")
                    sys.exit
            #print(wheel)
        ...
        #    print("after mutation pop size=",len(population))
          ...
          #      if best_flag:
            #            print("best pop found at gen=",generation," copying")
              #          bestpopulation=population.copy()
                #        best_gen=generation
                            #best_rowno=rowno
                  #      best_flag=False
