...
def calc_distance(start,finish):
  #  print("calc dist from",start,"to",finish)
    ...
    finishx=finish[0]
    finishy=finish[1]
  #  return(distance.cdist([start],[finish],'euclidean')[0][0])                              
       ...
       # print("genepool dist=",ts.totalgenepool_dist)
        ...
        #    print("")
          #  print("best journey=",ts.genepool[bestjourneyno],"dist=",bestjourneydist)
            ...
            print("\nPID:",pid,"Generation:",gencount," shortest distance:",round(bestjourneydist,2)," health:",probtable_set,"Mutation rate:",mutation_rate)  #,end='\r',flush=True)
            drawit=True
...
# load data into numpy data structure
      ...
      #  print("data out ready=",dataout)
        ...
        print("\rGen:",gencount,"   ",end="\r",flush=True)
        add_to_queue(dataout_as_bytes,q)
...
def main():
  ...
  #  ts.no_of_stops=0
    ...
    poolsize=200
    mutation_rate=[9,12,15,18,21,24,27,30,8,7]   # different mutation rates for the different parallel processes
