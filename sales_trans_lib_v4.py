...
class salestrans:
        ...
        for key in plot_dict.copy():
            ...
            #    mat_value=self.mat_add_1d(tf.transpose(tf_value, [1, 0]),self.mats) 
             ...
             #   print("plot disyvc items",plot_dict[mat_key].items())
              ...
              #      plt.plot_dict[mat_key]   #.plt(style="b")   # actual
          ...
          #      df[df.columns[2]].plot(yerr=df.stddev)
                    # plt.plot(range(len(plot_dict)), list(plot_dict.values()))
                    # plt.xticks(range(len(plot_dict)), list(plot_dict.keys()))

                    # plt.title("Noving average unit sales")   #str(new_plot_df.columns.get_level_values(0)))
                    # plt.legend(fontsize=11)
                    # plt.ylabel("units/day sales")
                    # plt.grid(True)
                    # self.save_fig("Moving Avedrage unit sales"+query_name,self.images_path)
 ...
 #         from scipy.ndimage.filters import uniform_filter1d
    #    return(tf.convert_to_tensor(uniform_filter1d(series[0], size=mat_days,mode='nearest')[tf.newaxis,...]))
      #  return(tf.convert_to_tensor(uniform_filter1d(series[0], size=mat_days)[tf.newaxis,...]))
 ...
 #        return output_2d[tf.newaxis,...]
    ...
    #    tf.print("bopb series shape",series.shape,"batchl",batch_length)
        start_points=tf.range(0,self.batch_length,dtype=tf.int32)  
        grid_indices=(tf.meshgrid(start_points,tf.range(0,series.shape[1]-batch_length+1,dtype=tf.int32)))   #int((end_point-start_point)/batch_length)+1))
        start_index=tf.convert_to_tensor(tf.add(grid_indices[0],grid_indices[1]))   #[:,:,np.newaxis
   #     tf.print("boab start_index",start_index,start_index.shape)
 ...
 #       start_points=tf.random.uniform(shape=[no_of_batches],minval=0,maxval=batches.shape[0]-batch_length-1,dtype=tf.int32)
        start_points=tf.random.uniform(shape=[no_of_batches],minval=self.start_point,maxval=self.end_point-batch_length+1,dtype=tf.int32)
     #   tf.print("start point=",start_point,"nob",no_of_batches,"batch len=",batch_length)
      ...
      #  tf.print("X indices",X_indices,X_indices.shape)
        tf.print("X ind min=",tf.reduce_min(X_indices),"max=",tf.reduce_max(X_indices))
        tf.print("s-e",self.start_point,self.end_point)
        tf.print("batches shape",batches.shape)
...
# @tf.function
  ...
  #      print("predict series series.shape",series.shape)
    ...
    #    predict_start_indices=(self.end_point-self.batch_length)
#        predict_ahead=model(tf.cast(series[:,-((self.batch_length*2)+(series.shape[1]-self.end_point)):,:],tf.float32),training=True)
        predict_ahead=model(tf.cast(series[:,self.start_point:self.end_point,:],tf.float32))

   #     print("1predict ahead shape",predict_ahead.shape)

        new_prediction=np.empty((1,0,1))
        new_stddev=np.empty((1,0,1),dtype=np.float32)

        for batch_ahead in range(0,self.predict_ahead_length,self.batch_length):        
    ...
    #            Y_probs=np.stack([model(predict_ahead[:,-batch_length:,:],training=True)[0,:,-1] for sample in range(pred_error_sample_size)])         
         #   predict_start_indices=(self.end_point-self.batch_length)
         #   print("predict ahead proir to model",predict_ahead[:,-self.one_year:,:].shape)              
 #           Y_probs=np.stack([model(predict_ahead[:,-self.one_year:,:],training=True)[0,:,-1:].numpy() for sample in range(self.pred_error_sample_size)])         
      #      Y_probs=np.stack([model(predict_ahead[:,-self.one_year:,:],training=True)[0,:,-1:].numpy() for sample in range(self.pred_error_sample_size)])         
 #           Y_probs=np.stack([model(predict_ahead[:,-self.batch_length:,:],training=True)[0,-1].numpy() for sample in range(self.pred_error_sample_size)])         
        ...
        #    print("before new prediction",new_prediction.shape)
          ...
          #  print("mafter new prediction concat with Y_mean=",new_prediction.shape)
         #   pa=predict_ahead[0,:,-1]
         #   print("2predict ahead shape",predict_ahead[:,:,-1:].shape)   #,"phs[:,:,-1:]",predict_ahead[:,:,-1:],
                  #"phs[:,:,-1:]",predict_ahead[:,:,-1:].shape)
            predict_ahead=np.concatenate((predict_ahead[:,:,-1:],Y_mean),axis=1)
          #  print("3predict ahead shape after concat with Y_mean",predict_ahead.shape)
