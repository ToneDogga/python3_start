...
def plot_stacked_line_pivot(pivot_df,title,stacked=True,number=6):    
 ...
 #   print("before plot",pivot_df) 
  ...
  #  print("plot pivot",pivot_df)
    ...
    pivot_df.plot(rot=45,grid=True,logy=False,use_index=True,fontsize=8,kind='line',stacked=stacked,title=title+", stacked="+str(stacked))
    plt.show()
    return
...
def plot_trend(s,title):
   #  ax=s[['days_since_last_order','units']].iloc[-1].plot(x='days_since_last_order', linestyle='None', color="green", marker='.')
     ...
     ax=s[['days_since_last_order','units']].iloc[:-1].plot(x='days_since_last_order', linestyle='None', color="red", marker='o')

     s[['days_since_last_order','bestfit']].plot(x='days_since_last_order',kind="line",ax=ax)
...
def calculate_first_derivative(s,cust,prod):
  ...
  #  s['days'].iloc[-1]=newgap
    ...
    #print("last date",sales_df['date'].iloc[-1])#-sales_df['date'].iloc[-1]
 ...
 #   s=s.sort_values('date',ascending=False)
  ...
  #  s['mat14']=s['qty'].rolling("14D",min_periods=1,axis=0).sum()
    #s['diff']=s.mat14.diff(periods=1)
    #s['14_day%']=round(s['diff']/s['mat14']*100,2)
 #   print(s)
    X=s[['days']].to_numpy()
    X=X[::-1,0]
  #  X=X[:,0]
...
#    print("p",p)
  ...
  #  print(s[['qty','daysdiff','X','y','days']])
  #  p = polyfit(x,y,1);
  #  d = polyval(p,x);
  #  plot(x,d,'k--')
 #   s['trend'] = np.concatenate((y_empty,np.polyval(p, X)),axis=0)
  #  print("p1",p1,"p1[0]",p1[0])
    ...
    if slope>0.06:
        ...
        plot_trend(s,title)
  #  if slope>0.1:
   #     ax=s.tail(365)[['date','qty']].plot(x='date',y='qty',kind='scatter',rot=30,grid=True,title=title)   #),'BB total scanned vs purchased Coles jam units per week')
   #     ax=s.tail(365)[['date','qty','bestfit']].plot(x='date',grid=True,title=title)   #),'BB total scanned vs purchased Coles jam units per week')
    #ax=s.tail(365)[['days','qty']].plot(x='days',y='qty',kind='scatter',rot=30,grid=True,title=title)   #),'BB total scanned vs purchased Coles jam units per week')
     #   ax=s.tail(365)[['days','qty','bestfit']].plot(x='days',grid=True,title=title)   #),'BB total scanned vs purchased Coles jam units per week')

    #    ax=s.tail(365)[['date','qty','90_day%','yp']].plot(x='date',y='yp',kind='scatter',grid=True,title=title)   #),'BB total scanned vs purchased Coles jam units per week')
...
plot_stacked_line_pivot(pivot_df,"Unit sales per month by productgroup",False)   #,number=6)
...
#pivot_df.sort_index(axis='columns', ascending=False, level=['month','year'])
plot_stacked_line_pivot(pivot_df,"Dollar sales per month by spc",False)   #,number=6)
