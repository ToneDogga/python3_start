...
def load_data(filename,index_code,required_length):    #,col_name_list,window_size):   #,mask_text):   #,batch_size,n_steps,n_inputs):   
    

    answer="n"
    answer=input("Load excel data from "+filename+"?")
    if answer=="y":
        df=pd.read_excel(filename,-1)  # -1 means all rows   
        pd.to_pickle(df,"raw_pivot_table.pkl")
    else:
        print("loading saved excel data - raw_pivot_table.pkl")
        df= pd.read_pickle("raw_pivot_table.pkl")
...
#    mask=(df['product']=="TS300") 
    mask=((df['code']=='FLNOR'))    #& (df['product']=='SJ300'))   #mask=bool(mask_str)
...
def extend_pivot_table(series_table,dates,periods_len,actual_days_in_series_table,required_starting_length,predict_ahead_steps,start_point): 
   ...
   # #     filename="allsalestrans020218-190320.xlsx"
    ...
    #   series2,product_names,periods=load_data(filename)    #,n_steps+1)  #,batch_size,n_steps,n_inputs)
     #   series2,product_names,ddates,table,mat_table,mat_sales,mat_sales_90=load_shop_data(filename)    #,n_steps+1)  #,batch_size,n_steps,n_inputs)
        series_table,dates=load_data(filename,index_code,required_starting_length)    #,col_name_list,window_size)    #,n_steps+1)  #,batch_size,n_steps,n_inputs)
    ...
    #        series_table.index = series_table.index.map(''.join).astype(str) 
     ...
     #   print("dates=",dates,len(dates))
        ...
        # print("load series table before",series_table.shape)
        # if actual_days_in_series_table>=required_starting_length:
        #     series_table=series_table.iloc[:,-required_starting_length:] 
        #     actual_days_in_series_table=required_starting_length
        #     print("truncated series_table to the last",required_starting_length,"days")
        # else:
        #     print("\nWarning... the number of days loaded (",actual_days_in_series_table,") is not enough for a synced 2 year analysis. requires (",required_starting_length,") days.\n")
        # print("load series table after",series_table.shape)

        
        #  this adds 

        
        series_table,extended_dates,actual_days_in_series,start_point=extend_pivot_table(series_table,dates,periods_len,actual_days_in_series_table,required_starting_length,predict_ahead_steps,start_point)
...
#    norm_y_train=tf.keras.utils.normalize(y_train, axis=-1, order=2)
 ...
 #########################################################
    ...
    if answer=="y":
      ...
      #      if (n_inputs>=12) & ((layer_count==3) | (layer_count==5)):  # | (layer_count==5) | (layer_count==6):
#            if ((layer_count==3) | (layer_count==5)):  # | (layer_count==4) | (layer_count==1) | (layer_count==6) | (layer_count==7)):
       ...
       #         model.add(keras.layers.BatchNormalization())
        ...
        #   if (layer_count==1):  # | (layer_count==5) | (layer_count==6):
#            if ((layer_count==3) | (layer_count==5)):  # | (layer_count==4) | (layer_count==1) | (layer_count==6) | (layer_count==7)):
        ...
        #    if (layer_count==2):  # | (layer_count==2)):   
         ...
         #        model.add(keras.layers.BatchNormalization())
          ...
          #  if (layer_count==7):  # | (layer_count==2)):   
          #       model.add(keras.layers.BatchNormalization())
...
#     f.write("\n\n"+table8.to_string())
    ...
    forecast_table.to_csv("SCB_"+s+".csv") 
