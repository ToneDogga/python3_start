...
##################################################################
 ...
 #   book=load_workbook(file)
    ...
    current_finish_date=current_start_date+relativedelta(weeks=26)
   # ws=wb.active
 ...
 #   worksheet = writer.sheets['report']
    ...
    while current_finish_date <= finishdate and current_start_date <=finishdate:
      ...
      #  print("dbg=\n",dbg)
        ...
        corr_matrix=dbh.corr()    #important_attributes)   #.sort_values(ascending=False)   #[important_attributes])   #important_attributes2,method="pearson")
       ...
       # print(sname,"\n\n")
        sdf=pd.DataFrame(columns=["Range","Type"])
       ...
       # print("sdf=",sdf)
        ...
        dbh=dbg[important_attributes]
        corr_matrix2=dbh.corr()    #important_attributes)   #.sort_values(ascending=False)   #[important_attributes])   #important_attributes2,method="pearson")

        print("\n\nCorrelations:\n",current_start_date,"to",current_finish_date,":\n",corr_matrix2)
      #  sname="c"+str(current_start_date.strftime('%Y%m%d'))+"_"+str(current_finish_date.strftime('%Y%m%d'))
       # print(sname,"\n\n")
        sdf2=pd.DataFrame(columns=["Range","Type"])
...
##        sarraydf2.to_excel(writer,sheet_name="Sheet2", startrow=start_row, index=True)
        ...
        if current_finish_date > finishdate:
