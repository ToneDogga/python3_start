...
#    print("sales_series.shape=",sales_series.shape)
    ...
    print("ys[:,1]=",ys[:,1])
 #   return sales_series.reshape(-1,n_steps,1), ys.reshape(-1,n_steps,1)
    return ys[:,:-1].reshape(-1,n_steps,1), ys[:,1:].reshape(-1,n_steps,1)
...
################################
 ...
 #   t = np.linspace(t_min, t_max, int((t_max - t_min) / resolution))
    ...
    m_start=8
    n_steps = 10
    monthno_instance = np.arange(m_start, m_start+n_steps).ravel()
...
#################################    
 ...
 #       X_new = time_series(np.array(t_instance[:-1].reshape(-1, n_steps, n_inputs)))
      ...
      #  X_new = sales[monthno_instance.reshape(-1, n_steps, n_inputs)]
