...
def calc_distances(stops,no_of_stops,poolsize,genepool,dist):
 #   clock_start=time.process_time()
    ...
    for i in range(0,poolsize-1):
        ...
        #    print("i=",i,"k=",k,"g=",ts.genepool[i][k])
            ...
            startx=stops[round(genepool[i][k])][0]
            starty=stops[round(genepool[i][k])][1]
            finishx=stops[round(genepool[i][k+1])][0]
            finishy=stops[round(genepool[i][k+1])][1]

            dist+=dist(finishx-startx,finishy-starty)
...
def ga_ts_run(xsize,ysize,stops,no_of_stops,poolsize,epoch_length,mutation_rate,q):
  ...
  #  print("genes=",ts.genepool)
    ...
    for gencount in range(1,epoch_length+1):
      ...
      #  print("calc total distance for each journey in genepool")
        distances,bestjourneyno=calc_distances(stops,no_of_stops,poolsize,genepool,dist)
   ...
   # print("lines per write=",linewrite," remainder=",lwremainder)
    ...
    #        multiple_results=[]
        ...
        for i in range(0,cpus-1):          
            multiple_results.append(pool.apply_async(ga_ts_run,args=(xsize,ysize,stops,no_of_stops,poolsize,epoch_length,mutation_rate[i],q )))  # stops, journey and poolsize, epoch length and name of q
 #           i+=1
