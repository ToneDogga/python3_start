...
payoff_filename="shoptrans2014to20193.csv"
number_of_cols=8   # rowno, 6 input vars and 1 out =8
linewidth=66   # 66 bytes
extra_EOL_char=0
row_find_method="l"
                   ...
                   # print("\np=",p," val=",val)
                    if direction=="x":  # maximising payoff
                        if p>max_payoff:
                             ...
                             #   pass
                             #else:
                                if not (self.pconstrain or self.aconstrain or self.bconstrain or self.cconstrain or self.dconstrain or self.econstrain or self.fconstrain):
                                    best=dna[count]
                                    bestrow=row_number     
                                    max_payoff=p
                                    found=True
 ...
 #       print("total number of bits=",totalbits)
   ...
   #    print("number of mutations needed=",number_of_mutations_needed)
    ...
    def genetic_algorithm(self):
        ...
        length=14   #16   # 16 length of dna bit strings
        max_payoff=-10000
        min_cost=10000
       ...
       #print("returned payoff:",returned_payoff)
                        if direction=="x":
                            if returned_payoff>max_payoff:
                                ...
                                calcpayoff=((2344*int(axis[2])-741)*0.23998+(-647*int(axis[3])+4343)*0.23998+(938*int(axis[4])+2071)*0.23998+(-9.8496*(int(axis[5])*int(axis[5]))+362*int(axis[5])-215)*0.19798+(-152*int(axis[6])+3725)*0.0816)
                                print("best fittest=",best," value=",int(best,2)," row no=",int(bestrow)," a=",axis[1]," b=",axis[2]," c=",axis[3]," d=",axis[4]," e=",axis[5]," f=",axis[6],"gen no:",gen,"max_payoff=",max_payoff,"calcp=",calcpayoff,flush=True)
                               
                                outfile.write("best fittest= "+best+" value= "+str(int(best,2))+" row no= "+str(int(bestrow))+" a="+str(axis[1])+" b="+str(axis[2])+" c="+str(axis[3])+" d="+str(axis[4])+" e="+str(axis[5])+" f="+str(axis[6])+" generation no: "+str(gen)+" max_payoff= "+str(max_payoff)+" calc payoff= "+str(calcpayoff)+"\n")
                            ...
                            if returned_payoff<min_cost:
                                ...
                                print("best fittest=",best," value=",int(best,2)," row no=",int(bestrow)," a=",axis[1]," b=",axis[2]," c=",axis[3]," d=",axis[4]," e=",axis[5]," f=",axis[6],"gen no:",gen,"min_cost=",min_cost,"calcp=",calcpayoff,flush=True)
                                outfile.write("best fittest="+best+" value="+str(int(best,2))+" row no="+str(int(bestrow))+" a="+str(axis[1])+" b="+str(axis[2])+" c="+str(axis[3])+" d="+str(axis[4])+" e="+str(axis[5])+" f="+str(axis[6])+" generation no: "+str(gen)+" min_cost= "+str(min_cost)+" calc payoff="+str(calcpayoff)+"\n")
...
if answer=="y":
    ...
    total_rows=generate_payoff_environment_6d_file(linewidth,0,2**3,0,2**1,0,2**1,0,2**1,0,2**6,0,2**3,payoff_filename)  
    clock_end=time.process_time()
    duration_clock=clock_end-clock_start
