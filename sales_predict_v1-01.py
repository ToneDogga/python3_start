...
#    print(dbd.columns)
   ...
   # dbd["qty"]=fulldataset["qty"]
    ...
    dbd.to_excel("dbd.xlsx")
 

################################################################################################    
  #  start_d=input("Start date for predictions? YYYY/MM/DD  (you can only predict 1 year in advance of current date):")
    start_d="2020/03/01"   #str(dt.datetime.now)   #input("Start date for predictions? YYYY/MM/DD  (you can only predict 1 year in advance of current date):")
    ...
    one_year=(dbd["date"]>=one_year_ago)    # & (df2.day_order_delta>=4) & (df2.day_order_delta<=45))


    
    dbd2=dbd[one_year].copy(deep=True)
 ...
 #   now_dt = datetime.strptime(now, '%Y/%m/%d').date()
  #  print("now=",now)
    #dbd2["new_date1"]=start_date  #.dt.date
    dbd2['new_date'] = start_date + pd.to_timedelta(dbd2['day_order_delta'], unit='d')
   ...
   # new_datetime_obj = datetime.strptime(orig_datetime_obj.strftime('%d-%m-%y'), '%d-%m-%y').date()
    ...
    dbd2= dbd2[["code","product","predict_date","qty","predict_qty"]]
    
    dbd2.sort_values(by=["predict_date"],axis=0,ascending=[True],inplace=True)
