...
def smoothing_distributions(df,prod_list,bins):
...
##
#### save to xlsx file
##    print("post-Scaler array saved to",cfg.scalerdump2)
##    sdf.to_excel(cfg.scalerdump2, index=False)
      ...
      #  i=df3[prod_mask].index
       ...
       #     df3[prod_mask].scaler.iloc[j] = scaler[i,abin]   #.apply(copy_scaler)
        ...
        #    df.set_value('Row_index', 'Column_name', value)
            df3.at[df3[prod_mask].index[j], 'prod_scaler']= scaled[k,abin]
            j+=1
        k+=1
      #  print("df5=\n",df4.head(100))
...
####################################################################3
    ...
    df3["scaled_upspd"]=df3["last_order_upspd"]*df3["prod_scaler"]*correction_factor    #cfg.sensitivity_constant
...
##    df3.to_excel("testing3.xlsx", index=False)
    ...
    return df3,active_bins,correction_factor
...
#    X_df=shift_reference(X_df,"months",-1,"last_month")
 ...
 #   print(X_df.tail(20).to_string())
  ...
  #  input("?")
    Xr_df,active_bins,correction_factor=order_delta(Xr_df)
 ...
 #   Xc_df.dropna(inplace=True)
    ...
    print("scale correction factor=",correction_factor)
