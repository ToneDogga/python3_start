...
#for qname in query_table.index:
    ...
    table_list[q].append(pd.pivot_table(df.query(quotes(query_dict[query_name]['cond'])), values='qty', index=query_dict[query_name]['index_code'].split(),columns=['period'], aggfunc=np.sum, margins=False,dropna=False,observed=False, fill_value=0).T) 
    q+=1
