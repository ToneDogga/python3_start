...
def next_batch(n_inputs, n_outputs,sales, batch_size, n_steps):
    sales_size=sales.shape[0]
    sales_product_count=sales.shape[1]
    print("sales.shape",sales.shape)
 #   print("sales rows, product_count=",sales_size,sales_product_count)
 #   start_array=np.random.randint(sales_product_count-batch_size-2,size=(n_inputs,1)) #* (sales.shape[0]-n_steps)
    start_array=np.random.randint(0,sales_product_count-n_steps,size=(n_inputs,1)) #* (sales.shape[0]-n_steps)
 ...
 #   print("start_array.shape=",start_array.shape)
    ...
    sales_series = start_array + np.arange(0, (n_steps/2)+1,dtype=int)


 #   print("sales series",sales_series)
#    print("sales_series before .shape=",sales_series.shape)
#    sales_series=sales_series.reshape(n_steps,n_outputs)
 #   sales_series=sales_series.T.reshape(n_steps,n_outputs)
    sales_series=sales_series.ravel()
 #   print("sales_series after.shape=",sales_series.shape)

    ys = sales[:,sales_series]

#    print("ys=",ys,ys.shape)
#    return ys[:,:,:-1].reshape(-1,n_steps,n_inputs), ys[:,:,1:].reshape(-1,n_steps,n_inputs)
    return ys[:,1:-1].reshape(-1,n_steps,n_inputs), ys[:,2:].reshape(-1,n_steps,n_inputs)
...
#    f.write("Results Reported to "+cfg.outfile_predict+"\n")
 ...
 #   df=read_excel(sys.argv[1],-1)  # -1 means all rows
    ...
    sales=get_sales()   # returns a numpy array of unit sales per week in each col, and each product in a row
...
################################
 ...
 #   n_steps = 20
 #   sales_instance = np.linspace(starttime, starttime + resolution * (n_steps + 1), n_steps + 1)
 ...
 #   n_layers=1
    ...
    sales_instance = np.arange(m_start, m_start+n_steps+1).reshape(n_inputs,)
    print("sales_instance=",sales_instance,"shape=",sales_instance.shape)
  ...
  #  print("\n\n")
    ...
    for prodrow in range(sales.shape[0]):

        plt.figure(figsize=(11,4))
        plt.subplot(121)
        plt.title("A unit sales series "+str(prodrow), fontsize=14)
       # for prodrow in range(0,sales.shape[0]):
   #     plt.plot(weekno, sales[prodrow,weekno], label=r"$unit sales$")
        plt.plot(sales_instance[:-1], sales[prodrow,sales_instance[:-1]], "b-", linewidth=3, label="A training instance")
     ...
     #   plt.axis([0, 30, -17, 13])
        ...
        plt.subplot(122)
        plt.title("A training instance "+str(prodrow), fontsize=14)
     #   for prodrow in range(0,sales.shape[0]):
        plt.plot(sales_instance[:-1], sales[prodrow,sales_instance[:-1]], "bo", markersize=10, label="instance")
        plt.plot(sales_instance[1:], sales[prodrow,sales_instance[1:]], "w*", markersize=10, label="target")
        plt.legend(loc="best")
        plt.xlabel("Week")
...
#################################    
   ...
   # batch_size = 52
    ...
    with tf.Session() as sess:
        ...
        for iteration in range(n_iterations):
            X_batch, y_batch = next_batch(n_inputs,n_outputs,sales, batch_size, n_steps)
   #         print("X_batch=\n",X_batch.shape,"\ny_batch=\n",y_batch.shape)
 ...
 #       X_new = time_series(np.array(t_instance[:-1].reshape(-1, n_steps, n_inputs)))
      ...
      #  print("X_index=",X_index)
        X_new = sales[:,sales_instance[:-1]].reshape(-1, n_steps, n_inputs)
  ...
  #  print("y_pred=",y_pred)
    ...
    for prodrow in range(sales.shape[0]):
        plt.title("Testing the model "+str(prodrow), fontsize=14)
        plt.plot(sales_instance[:-1], sales[prodrow,sales_instance[:-1]], "bo", markersize=10, label="instance")
        plt.plot(sales_instance[1:], sales[prodrow,sales_instance[1:]], "w*", markersize=10, label="target")
      #  plt.plot(weekno_instance, sales[weekno_instance], "w*", markersize=10, label="target")
    
        plt.plot(weekno_instance[1:], y_pred[0,:,prodrow], "r.", markersize=10, label="prediction")
        plt.legend(loc="best")
        plt.xlabel("Week")
...
##############################
   ...
   #     for prodrow in range(n_outputs):
             #  monthno=np.append(monthno,monthno.shape[0]+1)
          
        sequence = sales[:,-n_steps:]   #.reshape(-1, n_steps, n_inputs)
           #nexts=sequence[-1]+1
           #print("sq=",sequence,"nexts",nexts,"n_steps=",n_steps)
  #      print("starting sequence=\n",sequence.shape)
   
        for iteration in range(80):
    #       X_batch = sequence[prodrow,-n_steps:].reshape(1, n_steps, n_inputs)
           X_batch = sequence.reshape(-1, n_steps, n_inputs)
           y_pred = sess.run(outputs, feed_dict={X: X_batch})
         #  print("x_batch=\n",X_batch,"y_pred=\n",y_pred)
      #     print("y_pred=",y_pred,y_pred.shape)
        #   yp=y_pred[-1, -n_steps:, :].reshape(n_steps,n_outputs)
           sequence=np.concatenate((sequence[:,1:],y_pred[:,-1:]),axis=1)
 #          print("after sequence=\n",sequence.shape)
  ...
  #  print("y_pred=\n",y_pred[0,:,0]) 
    ...
    for prodrow in range(sales.shape[0]):
        display_sequence=sequence[prodrow,:]
  #      print("display_sequence=\n",display_sequence)

        plt.figure(figsize=(8,4))
        plt.title("Prediction "+str(prodrow), fontsize=14)
        plt.plot(np.arange(display_sequence.shape[0])+lensales, display_sequence, "b-", linewidth=1)
        plt.plot(weekno[:lensales], sales[prodrow,weekno[:lensales]], linewidth=3,label=r"$unit sales$")
        plt.plot(weekno[:n_steps], sales[prodrow,weekno[:n_steps]], linewidth=2, label=r"$unit sales$")
