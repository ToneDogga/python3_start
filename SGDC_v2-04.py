...
def order_delta(df):
    df.sort_values(by=["code","product","week_delta"],ascending=[True,True,False],inplace=True)
...
def main():
    ...
    infile="salestransslice5-NAT.xlsx"
    outfile="salestransslice5-NAT.csv"

    #infile="salestransslice4-FLNOR.xlsx"
    #outfile="salestransslice4-FLNOR.csv"
    print("Loading",infile,"into pandas for processing..")
    ...
    #print(df.head(10))
    #print("c=",df.shape)
    print("calculating and clean2 details....\n")
    df=week_delta(df)
...
#    df = df[["prod_encode","productgroup","week_of_year","order_delta","qty_bins"]]
...
##
##    grid_search = GridSearchCV(sgd_lr, parameters, n_jobs=-1, cv=3, iid=False, verbose=False)
##
...
##################################################
    print("pipeline SGDC, and Linear SVC")
#  (tol=1e-3)
    
    pipeline = Pipeline([
       # ('tfidf', TfidfVectorizer(stop_words='english')),
        ('clf', SGDClassifier(loss="log",tol=1e-3,learning_rate='constant',eta0=0.01)),      # max_iter=500
     #   ('svc', LinearSVC())
...
##        'tfidf__smooth_idf': (True, False),
    ...
    #    'clf__alpha': (0.000001, 0.00001, 0.0001, 0.001, 0.01),
        ...
        'clf__eta0':(0.01, 0.1, 1, 10, 100,1000),
        'clf__max_iter':(500,5000),
      #  'svc__kernel':['linear','rbf'],
  #      'svc__C':(0.1,1,10,100),
      #  'svc__C': (0.1, 1, 10, 100)
    }

    grid_search = GridSearchCV(pipeline, parameters_pipeline, n_jobs=-1, cv=3, iid=False, verbose=True)

    print("Performing SGD grid search...")
    print("pipeline:", [name for name, _ in pipeline.steps])
    print("parameters:")
    print(parameters_pipeline)
...
##
 ...
 #   grid_search = GridSearchCV(svc_linear, parameters, n_jobs=-1, cv=3)
    ...
    print(grid_search.best_params_)
    print(grid_search.best_score_)
    svc_sgdc_best = grid_search.best_estimator_
    accuracy = svc_sgdc_best.score(X_test, y_test)
    print('SGDC - The accuracy on testing set is: {0:.1f}%'.format(accuracy*100))

    print("Best score: %0.3f" % grid_search.best_score_)
    print("Best parameters set:")
    best_parameters = grid_search.best_estimator_.get_params()
    for param_name in sorted(parameters_pipeline.keys()):
    ...
    pipeline = Pipeline([
       # ('tfidf', TfidfVectorizer(stop_words='english')),
     #   ('clf', SGDClassifier(learning_rate='constant',eta0=0.01)),      # max_iter=500
        ('svc', LinearSVC())
      #  ('svc' , SVC(random_state=42)),
...
##        'clf__max_iter':(500,5000),
  ...
  #      'svc__C':(0.1,1,10,100),
    ...
    print("Performing LinearSVC grid search...")
    print("pipeline:", [name for name, _ in pipeline.steps])
    print("parameters:")
    print(parameters_pipeline)
    ...
    print("grid_search.best params_",grid_search.best_params_)
    print("grid_search.best_score_",grid_search.best_score_)
    svc_sgdc_best = grid_search.best_estimator_
    accuracy = svc_sgdc_best.score(X_test, y_test)
    print('LinearSVC - The accuracy on testing set is: {0:.1f}%'.format(accuracy*100))
...
##
    ...
    predictions=svc_sgdc_best.predict(X_test)
  #  pos_prob=prediction_prob[:,1]
    print("prediction prob:",predictions[0:300])      #:,1]

  #  print("pos_prob=\n",pos_prob)


  #  auc=svc_sgdc_best.score(X_test,predictions)
  #  print("score=",auc)
##
##    from sklearn.tree import DecisionTreeClassifier
