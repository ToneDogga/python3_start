...
###############################################################
              ...
              #  if val <= r.total_rows:   #total_rows:
                ...
                # indexerror needed here? 
                    ...
                    print("File IO error on ",r.payoff_filename,"finding fittest=",fittest)
                    sys.exit()


              #  print("plist=",plist)
              #  input("?")
                if pfound and len(plist)>=r.number_of_cols:
                    found=True
...
#                   print("\n\nFittest so far:",min_fittest,"best epoch:[",best_epoch,"] best generation in best epoch [",best_gen,"] best rowno [",best_rowno,"] min cost",min_payoff)
       ...
       #             r.outfile.write("Genepool size "+str(pop_size)+" * min_scaling factor "+str(min_scaling)+" / scaling factor "+str(scaling_factor)+" = actual scaling: "+str((pop_size*min_scaling)/scaling_factor)+"\n")
         ...
         #       r.outfile.write("Fittest so far "+str(best_fittest)+" best epoch "+str(best_epoch)+" best generation in best epoch "+str(best_gen)+" best row no "+str(best_rowno)+" min cost "+str(min_payoff)+"\n")
            ...
            mates,len_wheel=spin_the_mating_wheel(probability_table,population,r.pop_size,r.direction,r)  # wheel_len is the size of the unique gene pool to select from in the probability_table

               
            population=crossover(mates,r.no_of_alleles,individual,r)   # simple crossover
              

            population, mutation_count, whichchromo,whichmember=mutate(population,r.no_of_alleles,r.ploidy,r.pop_size,r.mutation_rate,r)   # 1000 means mutation 1 in a 1000 bits processed
...
def main():
    ...
    ga.pop_size=400  # population size
    ga.payoff_filename=sys.argv[1]  #  "shopsales4.csv"
    ...
    ga.direction="x"
    ga.scaling_factor=25  # 25 scaling figure is the last. this is related to the size of the genepool. this multiplies the payoff up so that diversity is not lost on the probability_table when probs are rounded
...
# length in bits of each input variable
    ...
    ga.alen=int(sys.argv[2])   # length in binary bits of each integer input
    ga.blen=int(sys.argv[3])
    ga.clen=int(sys.argv[4])
    ga.dlen=int(sys.argv[5])
    ga.elen=int(sys.argv[6])
    ga.flen=int(sys.argv[7])
    ga.glen=int(sys.argv[8])

    ga.no_of_alleles=ga.alen+ga.blen+ga.clen+ga.dlen+ga.elen+ga.flen+ga.glen  # length of each chromosome
