...
def order_delta(df):
    df.sort_values(by=["code_encode","prod_encode","day_delta"],ascending=[True,True,False],inplace=True)
##
 ...
 #   df["previous_qty"]=df["qty"].shift(periods=-1,fill_value=0.0)
  ...
  #  df["last_week_order_qty"]=df.query("day_order_delta<=7.0").qty.shift(periods=-1,fill_value=11.11)
    ...
    cust_list=list(set(df["code_encode"].tolist()))
    prod_list=list(set(df["prod_encode"].tolist()))
   # no_of_products=len(prod_list)
   # sales=Counter()
    for cust in cust_list:   
        for prod in prod_list:
            last_order=((df["prod_encode"]==prod) & (df["code_encode"]==cust))    # & (df2.day_order_delta>=4) & (df2.day_order_delta<=45))
...
##    print(l)   #ist(set(X_df.date.tolist())).sort()
 ...
 #   Xr_df=remove_poor_sellers(X_df,sales,cfg.minqty)
  ...
  #  X_df.drop(columns=["last_weeks_product","last_two_weeks_product","last_month_product","qty"],inplace=True)
    Xr_df.drop(columns=["last_order_qty","day_delta"],inplace=True)
