...
def calc_fitness(newpopulation,direction,max_fittest,max_payoff,min_fittest,min_payoff,best_rowno,payoff_filename):   # the whole population 
 ...
 #                       p=self.find_a_payoff2(val,payoff_filename)
   ...
   # print("calc fitness count=",count)   
    if direction=="x":
        return(newpopulation,found,rowno,fittest,returned_payoff,best_rowno,max_fittest,max_payoff)
    elif direction=="n":
        return(newpopulation,found,rowno,fittest,returned_payoff,best_rowno,min_fittest,min_payoff)
    else:
        print("direction error..")
        return(newpopulation,False,0,"",0,0,"",0)
...
def oldmutate(newpopulation,length,mutation_rate):
         mutation=False
         mutation_count=0
         temp=""
         gene_pool_size=len(newpopulation)*length
    ...
    for epoch in range(1,no_of_epochs+1):
        ...
        for generation in range(1,epoch_length+1):
         ...
         #   print("Epochs:",epoch+1,"Generation progress: [%d%%]" % (generation/epoch_length*100),"diversity (len_sel)=",len_sel,"Tot gene bits:%d" % total_genes,"Tot Mutations:%d      "  % (mutations), end='\r', flush=True)
            ...
            population,found,rowno,fittest,returned_payoff,best_rowno,max_fittest,max_payoff=calc_fitness(population,direction,max_fittest,max_payoff,min_fittest,min_payoff,best_rowno,payoff_filename)  # use linecache method for fin row no in payoff file.csv
     ...
     #       print(population)
      #      input("?")
            ...
            print("\rEpoch no:",epoch,"/",no_of_epochs,"  Generation progress: [%d%%]" % (generation/epoch_length*100)," fittest=",fittest," rowno=",rowno," returned payoff=",returned_payoff," best=",max_fittest,"bestrowno=",best_rowno," max_payoff=",max_payoff,"    ",end='\r',flush=True)
        ...
        print("")
    outfile.close()
    print("\n\nMutations=",mutations," Final population=")
    population,found,fittest,rowno,returned_payoff,best_rowno,max_fittest,max_payoff=calc_fitness(population,direction,max_fittest,max_payoff,min_fittest,min_payoff,best_rowno,payoff_filename)  # use linecache method for fin row no in payoff file.csv
